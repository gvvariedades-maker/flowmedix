/**
 * Pré-voo — capturas **before** do plano Editorial Premium Dashboard.
 * Saída: artifacts/editorial-premium-dashboard-before/
 *
 * Viewports do plano: 390×844 (mobile) e 1366×768 (desktop).
...
const OUT_DIR = path.join(
  process.cwd(),
  'artifacts/editorial-premium-dashboard-before',
);
...
async function gotoAndSettle(page: Page, url: string) {
  try {
    ...
    /* captura best-effort mesmo com navegação lenta / redirect */
  }
  await page.locator('body').waitFor({ state: 'attached', timeout: 10_000 }).catch(() => {});
  await page.waitForTimeout(1_200);
}

test.describe('Capture before — Editorial Premium Dashboard', () => {
