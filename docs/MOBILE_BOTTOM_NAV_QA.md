...
| 4 | `/estudar` | Último card da lista; paginação inline visível; filtros Banca/Assunto com sheet + teclado iOS não cobre «Fechar» |
...
| Fundo global iOS | Listras diagonais sem scroll bleed ao rolar páginas longas (`background-attachment: scroll` ≤767px) |
...
```bash
npm test -- __tests__/layout/mobileBottomNav.test.ts __tests__/layout/dashboardShellDesktopPadding.test.ts __tests__/layout/useBodyScrollLock.test.ts __tests__/layout/useMobileSheetKeyboardInset.test.ts
```
