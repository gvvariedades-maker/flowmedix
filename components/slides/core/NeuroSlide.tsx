...
import { getConceptMapBespoke } from '../registry/conceptMap';
...
export const NeuroSlideHub = ({
  ...
  switch (slide.type) {
    case 'concept_map': {
      ...
      const bespoke = getConceptMapBespoke(layoutVariant);
      if (bespoke) {
  ...
  if (normalizedData.type) {
    ...
    switch (normalizedData.layout_type) {
      case 'concept_map': {
        ...
        const bespoke = getConceptMapBespoke(legacyPresentation.layoutVariant);
        if (bespoke) {
          const Comp = bespoke.Component;
          inner = (
            <Comp
