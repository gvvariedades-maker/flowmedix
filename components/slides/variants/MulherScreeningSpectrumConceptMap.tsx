...
const SPECTRUM_ZONES: ScreeningSpectrumZone[] = [
  'hpv',
  'pre_screening',
  'active_screening',
  'trap_40',
...
interface MulherScreeningSpectrumConceptMapProps {
  concepts: ScreeningConcept[];
  theme: ThemeColors;
  footerRule?: string;
}
      ...
      <div className="relative h-2 rounded-full bg-gradient-to-r from-slate-200 via-pink-300 to-slate-300">
        {highlightAge !== null ? (
          <span
            ...
            style={{ left: `${((highlightAge - 18) / (64 - 18)) * 100}%` }}
            aria-hidden
          />
        ) : null}
      </div>
...
export function MulherScreeningSpectrumConceptMap({
  ...
  const grouped = useMemo(() => {
    ...
    for (const concept of concepts) {
      const inferred = inferScreeningZoneMarker(concept.title, concept.description);
  ...
  const [activeZone, setActiveZone] = useState<ScreeningSpectrumZone>(defaultZone);
  ...
  const focusAge = activeZone === 'trap_40' ? 40 : activeZone === 'active_screening' ? 25 : null;

  const selectZone = useCallback((zone: ScreeningSpectrumZone) => {
    setActiveZone(zone);
