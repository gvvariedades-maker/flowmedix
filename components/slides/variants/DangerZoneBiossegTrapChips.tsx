...
import {
  BIOSSEG_PRECAUTION_SLOTS,
  biossegPrecautionLabel,
  inferBiossegTrapRoutes,
  type BiossegPrecaution,
} from '@/lib/slides/biossegSlideUtils';

function PrecautionRail({
  trapRoutes,
  correctRoutes,
  revealed,
}: {
  trapRoutes: BiossegPrecaution[];
  correctRoutes: BiossegPrecaution[];
  revealed: boolean;
}) {
  return (
    <div
      className="flex items-center justify-between gap-1 rounded-xl border border-lime-200/80 bg-lime-50/50 px-2 py-2"
      aria-hidden
    >
      {BIOSSEG_PRECAUTION_SLOTS.map((route) => {
...
function TrapChipCard({
  ...
  const { trapRoutes, correctRoutes, hasRail } = inferBiossegTrapRoutes(
    label,
    trapText,
    correctText,
  );
      ...
      >
        <div className="grid grid-rows-[auto_auto_minmax(0,1fr)_auto] gap-2 p-4">
          <div className="flex items-start justify-between gap-2">
            ...
            >
              {isRevealed ? 'precaução correta' : `erro #${index + 1}`}
            </span>
          </div>

          {hasRail ? (
            <PrecautionRail
              trapRoutes={trapRoutes}
            ...
            <div className="flex items-center gap-2 rounded-xl border border-lime-200/70 bg-lime-50/60 px-3 py-2">
              <ShieldAlert className="h-4 w-4 shrink-0 text-lime-700" aria-hidden />
          ...
          {isRevealed ? (
            ...
            <span className="font-mono text-[9px] font-bold uppercase tracking-widest text-rose-500/80">
              Toque para revelar a precaução correta →
            </span>
          )}
        </div>
      </div>
...
interface DangerZoneBiossegTrapChipsProps {
  content: string;
...
export function DangerZoneBiossegTrapChips({
  content,
  items,
  theme,
  footerRule,
  compareRevealMode = 'auto',
}: DangerZoneBiossegTrapChipsProps) {
        ...
        {content ? (
          <div className="mb-4 flex justify-center">
            <div className={`rounded-full border px-5 py-2.5 ${theme.borderColor} ${theme.iconBg}`}>
              <p
                className={`font-display text-center text-sm font-extrabold uppercase tracking-wide ${theme.textPrimary}`}
              >
                {content}
              </p>
            </div>
