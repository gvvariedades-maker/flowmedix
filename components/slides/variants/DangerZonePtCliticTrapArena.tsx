...
const CHIP_STYLES: Record<PtCliticChip['tone'], string> = {
...
function ChipsRow({ chips, revealed }: { chips: PtCliticChip[]; revealed: boolean }) {
...
function LetterBadge({ letter }: { letter: string }) {
  return (
    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-sky-500 to-rose-500 font-display text-lg font-black text-white shadow-sm">
      {letter}
    </div>
  );
}
...
function TrapCard({
  ...
  const station = inferRailStation(`${label} ${trapText} ${correctText}`);
  const chips = stationChips(station);
      ...
      <div className="flex flex-col gap-3 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-mono text-[10px] font-bold uppercase tracking-wide text-rose-700">
              {stationBadge(station)} · pegadinha
            </p>
        ...
        {isRevealed && correctText ? (
          ...
          <span className="font-mono text-[9px] font-bold uppercase tracking-widest text-rose-500/80">
            Toque para ver o que o trilho exige →
          </span>
        ) : null}
      </div>
    </motion.button>
...
function TransferTrapCard({
  ...
  const trapText = item.detail || item.description || '';
  const correctText = typeof item.correct === 'string' ? item.correct.trim() : '';
  const quizAnswer = inferCliticQuizAnswer(correctText);
...
interface DangerZonePtCliticTrapArenaProps {
  content: string;
...
export function DangerZonePtCliticTrapArena({
  content,
  items,
  theme,
  footerRule,
  compareRevealMode = 'tap',
}: DangerZonePtCliticTrapArenaProps) {
  ...
  return (
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-2xl flex-col gap-4">
        {content ? (
          <div className="flex items-center justify-center gap-2 rounded-full border border-sky-200/80 bg-white/85 px-4 py-2 shadow-sm">
            <Waypoints className="h-4 w-4 text-sky-700" aria-hidden />
            <p className="font-mono text-[10px] font-bold uppercase tracking-widest text-sky-900">
              {content}
            </p>
          </div>
        ) : null}
        ...
        {footerRule ? (
          <p
            className={`rounded-xl border border-sky-200/80 bg-white/90 px-4 py-3 text-center font-body text-sm italic leading-relaxed text-sky-900/90 shadow-sm ${theme.textSecondary}`}
          >
            {footerRule}
          </p>
        ) : null}
