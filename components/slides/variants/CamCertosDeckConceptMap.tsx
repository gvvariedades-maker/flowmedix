...
import { inferCamCategory, inferVfChip, type CamCategory } from '@/lib/slides/camSlideUtils';

export interface CamCertoConcept {
  icon: string;
  title: string;
  description: string;
}
...
interface CamCertosDeckConceptMapProps {
  concepts: CamCertoConcept[];
  theme: ThemeColors;
}

export function CamCertosDeckConceptMap({ concepts, theme }: CamCertosDeckConceptMapProps) {
    ...
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 grid grid-cols-1 gap-3 sm:grid-cols-2 md:gap-4">
        {concepts.map((concept, index) => {
          const category = inferCamCategory(`${concept.title} ${concept.description}`);
          const meta = CATEGORY_META[category];
          const Icon = resolveLucideIcon(concept.icon);
          const expanded = expandedIndex === index;
          const hasLongText = concept.description.length > 64;
          const isFocus = category === 'certos' || category === 'regra_ouro';
          ...
          return (
            <motion.button
              ...
              className={`overflow-hidden rounded-[1.25rem] border border-slate-200/70 text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md border-l-[4px] ${meta.border} ${
                isFocus
                  ? `ring-2 ${meta.ring} bg-gradient-to-br from-white via-teal-50/40 to-emerald-50/50`
                  : 'bg-white/95'
              }`}
            >
