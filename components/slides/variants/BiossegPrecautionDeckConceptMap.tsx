...
import { inferBiossegPrecaution, type BiossegPrecaution } from '@/lib/slides/biossegSlideUtils';

export interface BiossegPrecautionConcept {
  icon: string;
  title: string;
  description: string;
}

const PRECAUTION_META: Record<
  BiossegPrecaution,
  { label: string; border: string; badge: string; badgeText: string; ring: string }
> = {
  padrao: {
    label: 'PADRÃO',
    border: 'border-l-lime-500/90',
  ...
  iras: {
    label: 'IRAS',
    border: 'border-l-green-500/90',
    badge: 'bg-green-100/90',
...
interface BiossegPrecautionDeckConceptMapProps {
  concepts: BiossegPrecautionConcept[];
  theme: ThemeColors;
  footerRule?: string;
}

export function BiossegPrecautionDeckConceptMap({
  concepts,
  theme,
}: BiossegPrecautionDeckConceptMapProps) {
    ...
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 grid grid-cols-1 gap-3 sm:grid-cols-2 md:gap-4">
        {concepts.map((concept, index) => {
          const precaution = inferBiossegPrecaution(`${concept.title} ${concept.description}`);
          const meta = PRECAUTION_META[precaution];
          const Icon = resolveLucideIcon(concept.icon);
          const expanded = expandedIndex === index;
          const hasLongText = concept.description.length > 64;
          const isFocus = precaution === 'gabarito';
          ...
          return (
            <motion.button
              ...
              className={`overflow-hidden rounded-[1.25rem] border border-slate-200/70 text-left shadow-sm transition-all duration-200 hover:-translate-y-0.5 hover:shadow-md border-l-[4px] ${meta.border} ${
                isFocus
                  ? `ring-2 ${meta.ring} bg-gradient-to-br from-white via-lime-50/40 to-emerald-50/50`
                  : 'bg-white/95'
              }`}
            >
