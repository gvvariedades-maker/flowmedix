...
> = {
  ...
  gabarito: {
    card: 'bg-gradient-to-br from-sky-100 via-cyan-50 to-sky-100 border-sky-300/70 shadow-sky-200/40',
...
function stepTitle(step: string, role: PtCliticStepRole): string {
  ...
  if (letter && role === 'validar_letra') return `Letra ${letter} — embarca`;
  if (letter && role === 'eliminar_letra') return `Letra ${letter} — barrada`;
  return 'Passo do trilho';
}

function iconForRole(role: PtCliticStepRole, station: PtCliticRailStation): string {
  ...
  if (role === 'eliminar_letra') {
    if (station === 'proclise') return 'ArrowLeft';
    if (station === 'infinitivo_participio') return 'Ban';
  ...
  return (
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-hidden">
      <div className="absolute inset-0 overflow-hidden bg-[#f0f9ff]">
        ...
        <div className="absolute bottom-[10%] left-[5%] h-72 w-72 rounded-full bg-blue-200/30 blur-[90px]" />
      </div>
      ...
      <div className="relative z-10 flex items-start justify-between px-6 pt-5">
        <div>
          <span className="inline-flex items-center gap-1.5 rounded-full border border-black/5 bg-white/80 px-3 py-1 font-mono text-[10px] font-bold uppercase tracking-widest text-sky-800 shadow-sm">
            <Waypoints className="h-3 w-3" aria-hidden />
            Trilho ativo
          </span>
          <h2 className="mt-2 font-display text-2xl font-black leading-tight text-slate-900">
            Atrativo? → <span className="text-sky-700">pró / ên / meso</span>
          </h2>
        </div>
          ...
          {remaining.length === 0 ? (
            ...
            <>
              <p className="font-display text-3xl font-black text-slate-900">
                {passed.length + 1}
                <span className="text-lg font-bold text-sky-700">/{normalizedSteps.length}</span>
              </p>
          ...
          {remaining.length === 0 ? (
            <div className="flex h-full min-h-[min(420px,52vh)] flex-col items-center justify-center gap-2 text-center">
              ...
              <p className="font-body text-base text-slate-500">
                Em similares: há atrativo? → pró. Sem? → ên. Particípio? sem ênclise.
              </p>
            </div>
          ) : (
            remaining.slice(0, 4).map((stepIndex, stackPos) => {
              ...
              const station = inferRailStation(step);
      ...
      {footerRule ? (
        <p className="relative z-10 mx-4 mb-4 rounded-xl border border-sky-200/80 bg-white/90 px-3 py-2 text-center font-body text-xs italic text-sky-900/90 shadow-sm md:mx-6">
          {footerRule}
        </p>
      ) : null}
    </div>
