...
import { Check, Hand, HeartPulse, MapPin, Wind, X } from 'lucide-react';
import type { ThemeColors } from '../core/themeGenerator';
import type { DangerZoneItem } from './DangerZone';
import {
  inferUrgenciasChokingTrapSlot,
  urgenciasChokingTrapSlotLabel,
  URGENCIAS_CHOKING_TRAP_SLOTS,
  type UrgenciasChokingTrapSlot,
} from '@/lib/slides/urgenciasEngasgoSlideUtils';

type TrapEntry = {
  slot: UrgenciasChokingTrapSlot;
  index: number;
  label: string;
  detail: string;
  correct: string;
...
const SLOT_META: Record<
  ...
  { label: string; tag: string; bar: string; ring: string; panel: string; text: string; icon: typeof Wind }
> = {
  local_corpo: {
    label: 'Local',
    tag: 'anatomia',
...
function buildEntries(items: DangerZoneItem[]): TrapEntry[] {
  return items.map((item, index) => {
    ...
    const slot = inferUrgenciasChokingTrapSlot(label, detail, correct);
    return { slot, index, label, detail, correct };
  });
}

interface DangerZoneUrgenciasChokingTrapArenaProps {
  content: string;
...
export function DangerZoneUrgenciasChokingTrapArena({
  content,
  items,
  theme,
  footerRule,
}: DangerZoneUrgenciasChokingTrapArenaProps) {
  const reduceMotion = useReducedMotion();
  const entries = useMemo(() => buildEntries(items), [items]);
  const [activeSlot, setActiveSlot] = useState<UrgenciasChokingTrapSlot>(
    () => entries[0]?.slot ?? 'local_corpo',
  );
  ...
  const slotsWithItems = useMemo(() => {
    const seen = new Set<UrgenciasChokingTrapSlot>();
    for (const e of entries) seen.add(e.slot);
    return URGENCIAS_CHOKING_TRAP_SLOTS.filter((s) => seen.has(s));
  }, [entries]);
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    <p className={`font-mono text-[9px] font-extrabold uppercase tracking-widest ${meta.text}`}>
                      {urgenciasChokingTrapSlotLabel(activeSlot)} · {meta.tag}
                    </p>
                ...
                <button
                  ...
                  className={`w-full rounded-xl border px-3 py-3 text-left transition-all ${
                    revealed.has(activeEntry.index)
                      ...
                      : 'border-cyan-300/80 bg-cyan-50/90 hover:shadow-sm active:scale-[0.99]'
                  }`}
                >
