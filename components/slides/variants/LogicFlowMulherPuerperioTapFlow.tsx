...
import {
  parseMulherPuerperioStep,
  puerperioSlotLabel,
  type ParsedPuerperioStep,
...
function PuerperioSlotChips({ slots }: { slots: ParsedPuerperioStep['slots'] }) {
...
function StepBadge({ step }: { step: ParsedPuerperioStep }) {
  const styles: Record<ParsedPuerperioStep['kind'], string> = {
    anchor_timeline: 'bg-sky-100 text-sky-900',
...
export function LogicFlowMulherPuerperioTapFlow({
  ...
  const parsedSteps = useMemo(
    () => normalizedSteps.map((step, index) => parseMulherPuerperioStep(step, index)),
    [normalizedSteps],
  );
        ...
        <AnimatePresence mode="wait">
          ...
          >
            <div className="mb-3 flex flex-col items-center gap-2">
              ...
              <PuerperioSlotChips slots={current.slots} />
            </div>
