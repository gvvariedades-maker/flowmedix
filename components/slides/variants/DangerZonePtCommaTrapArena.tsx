...
import { Check, Pilcrow, X } from 'lucide-react';
...
const CHIP_STYLES: Record<PtCommaChip['tone'], string> = {
  violet: 'bg-violet-100/90 text-violet-900 ring-violet-300/50',
...
function ChipsRow({ chips, revealed }: { chips: PtCommaChip[]; revealed: boolean }) {
...
function LetterBadge({ letter }: { letter: string }) {
  return (
    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-violet-500 to-rose-500 font-display text-lg font-black text-white shadow-sm">
      {letter}
    </div>
  );
}
...
function TrapCard({
  ...
  const station = inferCommaStation(`${label} ${trapText} ${correctText}`);
  const chips = commaStationChips(station);
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-mono text-[10px] font-bold uppercase tracking-wide text-rose-700">
              {commaStationBadge(station)} · pegadinha
            </p>
...
interface DangerZonePtCommaTrapArenaProps {
  content: string;
...
export function DangerZonePtCommaTrapArena({
  content,
  items,
  theme,
  footerRule,
  compareRevealMode = 'tap',
}: DangerZonePtCommaTrapArenaProps) {
  ...
  return (
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-2xl flex-col gap-4">
        ...
        {footerRule ? (
          <p
            className={`rounded-xl border border-violet-200/80 bg-white/90 px-4 py-3 text-center font-body text-sm italic leading-relaxed text-violet-900/90 shadow-sm ${theme.textSecondary}`}
          >
            {footerRule}
          </p>
        ) : null}
