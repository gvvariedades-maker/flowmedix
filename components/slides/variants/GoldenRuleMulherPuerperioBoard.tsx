              ...
              <span className="min-w-0 flex-1">
                ...
                {rowSlots.length > 0 ? (
                  <div className="mb-1.5 flex flex-wrap gap-1">
                    {rowSlots.map((slot) => (
                      <CategoryStrip key={slot} label={puerperioSlotLabel(slot)} tone="command" />
                    ))}
                  </div>
                ) : null}
...
/** Puerpério board — LabelBodyRow + PuerperioTimelineRail (Fábrica G2). */
export function GoldenRuleMulherPuerperioBoard({
  ...
  const eyebrow =
    title && title.length <= 72 ? title : title ? 'Puerpério — MS/AB' : 'Puerpério · marcos';
