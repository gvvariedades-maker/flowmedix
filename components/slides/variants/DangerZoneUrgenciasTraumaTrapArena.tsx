...
import {
  ...
  URGENCIAS_TRAUMA_TRAP_SLOTS,
  type UrgenciasTraumaTrapSlot,
} from '@/lib/slides/urgenciasTraumaSlideUtils';

type TrapEntry = {
  slot: UrgenciasTraumaTrapSlot;
  index: number;
  label: string;
  detail: string;
  correct: string;
...
function buildEntries(items: DangerZoneItem[]): TrapEntry[] {
  return items.map((item, index) => {
    ...
    const slot = inferUrgenciasTraumaTrapSlot(label, detail, correct);
    return { slot, index, label, detail, correct, fixation: correct };
  });
}

interface DangerZoneUrgenciasTraumaTrapArenaProps {
  content: string;
...
export function DangerZoneUrgenciasTraumaTrapArena({
  content,
  items,
  theme,
  footerRule,
}: DangerZoneUrgenciasTraumaTrapArenaProps) {
  const reduceMotion = useReducedMotion();
  const entries = useMemo(() => buildEntries(items), [items]);
  const [activeSlot, setActiveSlot] = useState<UrgenciasTraumaTrapSlot>(
    () => entries[0]?.slot ?? 'hemorragia',
  );
  ...
  const slotsWithItems = useMemo(() => {
    const seen = new Set<UrgenciasTraumaTrapSlot>();
    for (const e of entries) seen.add(e.slot);
    return URGENCIAS_TRAUMA_TRAP_SLOTS.filter((s) => seen.has(s));
  }, [entries]);
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    <p className={`font-mono text-[9px] font-extrabold uppercase tracking-widest ${meta.text}`}>
                      {urgenciasTraumaTrapSlotLabel(activeSlot)} · {meta.tag}
                    </p>
                ...
                <button
                  ...
                  className={`w-full rounded-xl border px-3 py-3 text-left transition-all ${
                    revealed.has(activeEntry.index)
                      ...
                      : 'border-orange-300/80 bg-orange-50/90 hover:shadow-sm active:scale-[0.99]'
                  }`}
                >
