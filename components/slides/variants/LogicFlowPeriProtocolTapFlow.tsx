...
function StepBadge({ step }: { step: ParsedPeriStep }) {
  const styles: Record<ParsedPeriStep['kind'], string> = {
    ...
    locate: 'bg-indigo-100 text-indigo-900',
...
function ProtocolSlotChips({ slots }: { slots: ParsedPeriStep['protocolSlots'] }) {
...
export function LogicFlowPeriProtocolTapFlow({
  ...
  const parsed = useMemo(
    () => normalizedSteps.map((text) => parsePeriProtocolStep(text)),
    [normalizedSteps],
  );
        ...
        {parsed.map((step, index) => {
          ...
          return (
            ...
            >
              ...
              {revealed ? (
                <>
                  <ProtocolSlotChips slots={step.protocolSlots} />
