...
export interface PtCliticRailConcept {
  icon: string;
  title: string;
  description: string;
}
...
> = {
  definicao: {
    ...
    accent: 'from-sky-50 via-white to-sky-50/60',
  },
  pergunta_atrativo: {
    label: 'PORTÃO',
  ...
  infinitivo_participio: {
    ...
    badge: 'bg-cyan-100/95',
    badgeText: 'text-cyan-900',
    ring: 'ring-cyan-400/30',
    accent: 'from-cyan-50 via-white to-cyan-50/60',
  },
  pegadinha: {
    label: 'PEGADINHA',
...
interface PtCliticRailDeckConceptMapProps {
  concepts: PtCliticRailConcept[];
  theme: ThemeColors;
  footerRule?: string;
}

export function PtCliticRailDeckConceptMap({
  concepts,
  theme,
  footerRule,
}: PtCliticRailDeckConceptMapProps) {
  ...
  const enriched = useMemo(
    () =>
      concepts.map((concept, index) => {
        const station = inferRailStation(`${concept.title} ${concept.description}`);
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-2xl flex-col gap-3">
        <div className="flex items-center justify-center gap-2 rounded-full border border-sky-200/80 bg-white/85 px-4 py-2 shadow-sm">
          ...
          <p className="font-mono text-[10px] font-bold uppercase tracking-widest text-sky-900">
            Trilho clítico — pró · ên · meso + atrativo?
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {enriched.map(({ concept, station, meta, index }) => {
            ...
            const isFocus =
              station === 'pergunta_atrativo' ||
              station === 'pegadinha' ||
              station === 'infinitivo_participio';
              ...
              >
                <div className="flex flex-col gap-2 p-4 md:p-5">
                  ...
                  {!expanded && hasLongText && !reduceMotion ? (
                    <span className="mt-1 inline-flex items-center gap-1 rounded-full bg-sky-100/70 px-2 py-0.5 font-mono text-[9px] font-bold uppercase tracking-widest text-sky-800">
        ...
        {footerRule ? (
          <p className="rounded-xl border border-sky-200/80 bg-white/90 px-4 py-3 text-center font-body text-sm italic leading-relaxed text-sky-900/90 shadow-sm">
            {footerRule}
          </p>
        ) : null}
      </div>
