...
const ZONE_META: Record<MamaSpectrumZone, { ring: string; panel: string; text: string }> = {
  autoexame_awareness: {
...
/** Régua etária local — escala 40–69 (rastreio mama). */
  ...
  return (
    <div className="relative rounded-xl border border-pink-200/80 bg-white/90 px-3 py-3">
      <div className="relative h-2 rounded-full bg-gradient-to-r from-slate-200 via-pink-300 to-slate-300">
        ...
        />
        <span className="absolute top-1/2 h-full w-0.5 -translate-y-1/2 bg-pink-600/40" style={{ left: '100%' }} />
      </div>
...
/** Espectro de rastreio mama — BoardChrome + LabelBodyRow (Fábrica G2). */
export function MulherMammographySpectrumConceptMap({
  ...
  const focusAge = activeZone === 'trap_40' ? 40 : activeZone === 'active_screening' ? 50 : null;
  ...
  return (
    <BoardChrome
      theme={theme}
      eyebrow="Mamografia · espectro 50–69"
