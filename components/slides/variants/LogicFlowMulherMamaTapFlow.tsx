...
import { Check, ChevronRight, Ribbon, X } from 'lucide-react';
...
function ZoneChips({ zones }: { zones: ParsedMamaStep['zones'] }) {
...
function StepBadge({ step }: { step: ParsedMamaStep }) {
  const styles: Record<ParsedMamaStep['kind'], string> = {
...
export function LogicFlowMulherMamaTapFlow({
  ...
  const parsedSteps = useMemo(
    () => normalizedSteps.map((step, index) => parseMulherMamaStep(step, index)),
    [normalizedSteps],
  );
    ...
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col gap-3">
        <div className="flex items-center justify-center gap-2">
          <Ribbon className="h-4 w-4 text-pink-400" aria-hidden />
