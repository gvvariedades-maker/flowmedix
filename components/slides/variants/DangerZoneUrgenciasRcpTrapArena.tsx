...
import { Activity, Check, HeartPulse, Wind, X, Zap } from 'lucide-react';
import type { ThemeColors } from '../core/themeGenerator';
import type { DangerZoneItem } from './DangerZone';
import {
  inferUrgenciasRcpTrapSlot,
  urgenciasRcpTrapSlotLabel,
  URGENCIAS_RCP_TRAP_SLOTS,
  type UrgenciasRcpTrapSlot,
} from '@/lib/slides/urgenciasSlideUtils';

type TrapEntry = {
  slot: UrgenciasRcpTrapSlot;
  index: number;
  label: string;
  detail: string;
  correct: string;
...
> = {
  ...
  dea_atraso: {
    ...
    panel: 'from-cyan-50/95 via-white to-sky-50/90',
    text: 'text-cyan-900',
    icon: Zap,
  },
  hiperventilacao: {
...
function buildEntries(items: DangerZoneItem[]): TrapEntry[] {
  return items.map((item, index) => {
    ...
    const slot = inferUrgenciasRcpTrapSlot(label, detail, correct);
    return {
      slot,
      index,
      label,
      ...
      correct,
      fixation: correct,
      critical: slot === 'pulso_intervalo' || slot === 'frequencia' || slot === 'profundidade',
    };
  });
}

interface DangerZoneUrgenciasRcpTrapArenaProps {
  content: string;
...
export function DangerZoneUrgenciasRcpTrapArena({
  content,
  items,
  theme,
  footerRule,
}: DangerZoneUrgenciasRcpTrapArenaProps) {
  const reduceMotion = useReducedMotion();
  const entries = useMemo(() => buildEntries(items), [items]);
  const [activeSlot, setActiveSlot] = useState<UrgenciasRcpTrapSlot>(
    () => entries[0]?.slot ?? 'pulso_intervalo',
  );
  ...
  const slotsWithItems = useMemo(() => {
    const seen = new Set<UrgenciasRcpTrapSlot>();
    for (const e of entries) seen.add(e.slot);
    return URGENCIAS_RCP_TRAP_SLOTS.filter((s) => seen.has(s));
  }, [entries]);
    ...
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-4 md:p-6">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-lg flex-col gap-3.5">
        <div className="flex flex-col gap-1.5">
          <span className="inline-flex w-fit items-center gap-1.5 rounded-full border border-rose-200/80 bg-white/90 px-2.5 py-1 font-mono text-[9px] font-bold uppercase tracking-widest text-rose-800 shadow-sm">
          ...
          <p className="flex items-center gap-1.5 font-body text-xs font-medium text-slate-600">
            <Activity className="h-3.5 w-3.5 shrink-0 text-rose-600" aria-hidden />
            Escolha o eixo da pegadinha e toque para revelar a conduta correta
          </p>
        </div>

        {slotsWithItems.length > 1 ? (
          <div className="flex flex-wrap items-center justify-center gap-1 rounded-xl border border-rose-200/70 bg-rose-50/50 px-2 py-2">
            {slotsWithItems.map((slot, i) => {
              ...
              return (
                <div key={slot} className="flex min-w-0 items-center gap-1">
                  ...
                  >
                    <span
                      className={`font-mono text-[8px] font-black uppercase leading-tight ${isActive ? 'text-rose-900' : 'text-slate-500'}`}
                    >
                      {urgenciasRcpTrapSlotLabel(slot)}
                    </span>
                  </button>
                  {i < slotsWithItems.length - 1 ? (
                    <span className="font-mono text-[10px] text-rose-400/80" aria-hidden>
                      →
                    </span>
                  ) : null}
                </div>
              ...
              <button
                ...
                className={`flex min-h-11 cursor-pointer flex-col items-center justify-center gap-1 rounded-2xl border px-1 py-2 transition-all ${
                  isActive
                    ...
                    : 'border-slate-200/90 bg-white/80 shadow-sm hover:shadow-md hover:ring-1 hover:ring-rose-200/80 active:scale-[0.98]'
                }`}
              >
                <span
            ...
            >
              ...
              <div className="space-y-3 px-4 py-3">
                ...
                {revealed.has(activeEntry.index) && activeEntry.fixation ? (
                  <p className="rounded-lg border border-rose-200/70 bg-white/80 px-3 py-2 font-body text-xs text-rose-900/90">
