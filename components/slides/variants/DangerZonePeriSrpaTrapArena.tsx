...
function inferPeriSrpaTrapLanes(
  label: string,
  detail: string,
  correct: string,
): { trapSlots: PeriSrpaSlot[]; correctSlots: PeriSrpaSlot[]; hasRail: boolean } {
  const trapSlot = inferPeriSrpaSlot(label, detail);
  const correctSlot = inferPeriSrpaSlot(label, correct);
...
function SrpaSlotRail({
  ...
  trapSlots: PeriSrpaSlot[];
  correctSlots: PeriSrpaSlot[];
  revealed: boolean;
}) {
  return (
    <div
      className="flex flex-wrap items-center justify-center gap-1 rounded-xl border border-fuchsia-200/80 bg-fuchsia-50/50 px-2 py-2"
      aria-hidden
    >
...
interface DangerZonePeriSrpaTrapArenaProps {
  content: string;
...
export function DangerZonePeriSrpaTrapArena({
  content,
  items = [],
  theme,
  footerRule,
  compareRevealMode,
}: DangerZonePeriSrpaTrapArenaProps) {
    ...
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 flex flex-col gap-3">
        ...
        {items.map((item, index) => {
          ...
          const { trapSlots, correctSlots, hasRail } = inferPeriSrpaTrapLanes(
            label,
            trapText,
            correctText,
          );
            ...
            >
              <div className="flex flex-col gap-3 p-4">
                <div className="flex items-start gap-3">
                  {letter ? (
                    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-rose-500 to-fuchsia-500 font-display text-lg font-black text-white">
                      {letter}
                    </div>
                  ) : null}
                ...
                {hasRail ? (
                  <SrpaSlotRail trapSlots={trapSlots} correctSlots={correctSlots} revealed={isRevealed} />
                ) : (
                  <div className="flex items-center gap-2 rounded-xl border border-fuchsia-200/70 bg-fuchsia-50/60 px-3 py-2">
                    <Activity className="h-4 w-4 shrink-0 text-fuchsia-700" aria-hidden />
                    <span className="font-mono text-[9px] font-bold uppercase tracking-widest text-fuchsia-800">
                      monitorização SRPA
                    </span>
                  </div>
                )}
