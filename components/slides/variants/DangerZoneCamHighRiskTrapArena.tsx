...
function LetterBadge({ letter }: { letter: string }) {
  return (
    <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl bg-gradient-to-br from-amber-500 to-orange-600 font-display text-lg font-black text-white shadow-sm">
      {letter}
    </div>
  );
}
...
function TrapCard({
  ...
  const { chips, hasChips } = inferHighRiskTrapSlots(label, trapText, correctText);
    ...
    <motion.button
      ...
      className={`w-full overflow-hidden rounded-[1.25rem] border text-left transition-all ${
        isRevealed
          ...
          : 'border-amber-200/80 bg-white/95 shadow-sm hover:border-amber-300'
      }`}
    >
      <div className="flex flex-col gap-3 p-4">
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="font-mono text-[10px] font-bold uppercase tracking-wide text-amber-800">Pegadinha</p>
          ...
          <span
            className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-full ${
              isRevealed ? 'bg-emerald-500 text-white' : 'bg-amber-100 text-amber-700'
            }`}
          >
        ...
        {isRevealed && correctText ? (
          ...
          <span className="font-mono text-[9px] font-bold uppercase tracking-widest text-amber-600/80">
...
interface DangerZoneCamHighRiskTrapArenaProps {
  content: string;
...
export function DangerZoneCamHighRiskTrapArena({
  content,
  items,
  theme,
  footerRule,
  compareRevealMode = 'tap',
}: DangerZoneCamHighRiskTrapArenaProps) {
  ...
  return (
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-2xl flex-col gap-4">
        {content ? (
          <div className="flex items-center justify-center gap-2 rounded-full border border-amber-200/80 bg-white/80 px-4 py-2 shadow-sm">
            <ShieldAlert className="h-4 w-4 text-amber-600" aria-hidden />
            <p className="font-mono text-[10px] font-bold uppercase tracking-widest text-amber-900">{content}</p>
          </div>
        ) : null}
