...
export interface PtTermMatrixConcept {
  icon: string;
  title: string;
  description: string;
}
...
> = {
  definicao: {
    ...
    badge: 'bg-teal-100/95',
    badgeText: 'text-teal-900',
    ring: 'ring-teal-400/30',
    accent: 'from-teal-50 via-white to-teal-50/60',
  },
  pergunta_teste: {
    label: 'PORTÃO',
...
interface PtTermMatrixDeckConceptMapProps {
  concepts: PtTermMatrixConcept[];
  theme: ThemeColors;
  footerRule?: string;
}

export function PtTermMatrixDeckConceptMap({
  concepts,
  theme,
  footerRule,
}: PtTermMatrixDeckConceptMapProps) {
  ...
  const enriched = useMemo(
    () =>
      concepts.map((concept, index) => {
        const cell = inferTermMatrixCell(`${concept.title} ${concept.description}`);
        return { concept, cell, meta: CELL_META[cell], index };
      }),
    [concepts],
  );
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-2xl flex-col gap-3">
        <div className="flex items-center justify-center gap-2 rounded-full border border-teal-200/80 bg-white/85 px-4 py-2 shadow-sm">
          ...
          <p className="font-mono text-[10px] font-bold uppercase tracking-widest text-teal-900">
            Matriz de cargos — verbo? · nome? · de quê?
          </p>
        </div>

        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {enriched.map(({ concept, cell, meta, index }) => {
            const Icon = resolveLucideIcon(concept.icon || 'HelpCircle');
            const expanded = expandedIndex === index;
            const hasLongText = concept.description.length > 60;
            const isFocus =
              cell === 'pergunta_teste' || cell === 'pegadinha' || cell === 'adj_adv';
              ...
              >
                <div className="flex flex-col gap-2 p-4 md:p-5">
                  ...
                  {!expanded && hasLongText && !reduceMotion ? (
                    <span className="mt-1 inline-flex items-center gap-1 rounded-full bg-teal-100/70 px-2 py-0.5 font-mono text-[9px] font-bold uppercase tracking-widest text-teal-800">
        ...
        {footerRule ? (
          <p className="rounded-xl border border-teal-200/80 bg-white/90 px-4 py-3 text-center font-body text-sm italic leading-relaxed text-teal-900/90 shadow-sm">
            {footerRule}
          </p>
        ) : null}
      </div>
