...
import { AlertTriangle, Check, ClipboardList, Flame, Pill, X } from 'lucide-react';
import type { ThemeColors } from '../core/themeGenerator';
import type { DangerZoneItem } from './DangerZone';
import {
  inferUrgenciasProtocolTrapSlot,
  urgenciasProtocolTrapSlotLabel,
  URGENCIAS_PROTOCOL_TRAP_SLOTS,
  type UrgenciasProtocolTrapSlot,
} from '@/lib/slides/urgenciasProtocolSlideUtils';

type TrapEntry = {
  slot: UrgenciasProtocolTrapSlot;
  index: number;
  label: string;
  detail: string;
  correct: string;
...
function buildEntries(items: DangerZoneItem[]): TrapEntry[] {
  return items.map((item, index) => {
    ...
    const slot = inferUrgenciasProtocolTrapSlot(label, detail, correct);
    return { slot, index, label, detail, correct };
  });
}

interface DangerZoneUrgenciasProtocolTrapArenaProps {
  content: string;
...
export function DangerZoneUrgenciasProtocolTrapArena({
  content,
  items,
  theme,
  footerRule,
}: DangerZoneUrgenciasProtocolTrapArenaProps) {
  const reduceMotion = useReducedMotion();
  const entries = useMemo(() => buildEntries(items), [items]);
  const [activeSlot, setActiveSlot] = useState<UrgenciasProtocolTrapSlot>(
    () => entries[0]?.slot ?? 'conduta_mito',
  );
  ...
  const slotsWithItems = useMemo(() => {
    const seen = new Set<UrgenciasProtocolTrapSlot>();
    for (const e of entries) seen.add(e.slot);
    return URGENCIAS_PROTOCOL_TRAP_SLOTS.filter((s) => seen.has(s));
  }, [entries]);
                ...
                <div className="flex items-center gap-2.5">
                  ...
                  <div className="min-w-0">
                    <p className={`font-mono text-[9px] font-extrabold uppercase tracking-widest ${meta.text}`}>
                      {urgenciasProtocolTrapSlotLabel(activeSlot)} · {meta.tag}
                    </p>
