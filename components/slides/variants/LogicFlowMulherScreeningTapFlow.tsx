...
import { Check, ChevronRight, Microscope, X } from 'lucide-react';
import type { ThemeColors } from '../core/themeGenerator';
import { normalizeLogicFlowSteps } from '@/lib/reverseStudySlidesNormalize';
import {
  parseMulherScreeningStep,
  screeningZoneLabel,
  type ParsedScreeningStep,
...
function ZoneChips({ zones }: { zones: ParsedScreeningStep['zones'] }) {
...
function StepBadge({ step }: { step: ParsedScreeningStep }) {
  const styles: Record<ParsedScreeningStep['kind'], string> = {
...
export function LogicFlowMulherScreeningTapFlow({
  ...
  const parsedSteps = useMemo(
    () => normalizedSteps.map((step, index) => parseMulherScreeningStep(step, index)),
    [normalizedSteps],
  );
    ...
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col gap-3">
        <div className="flex items-center justify-center gap-2">
          <Microscope className="h-4 w-4 text-pink-400" aria-hidden />
          <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-pink-200">
            Eliminação por idade/intervalo — toque para avançar
          </span>
        </div>
