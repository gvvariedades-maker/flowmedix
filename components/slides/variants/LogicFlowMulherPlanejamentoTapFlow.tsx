...
import {
  parseMulherPlanejamentoStep,
  planejamentoZoneShort,
  type ParsedPlanejamentoStep,
...
function ZoneChips({ zones }: { zones: ParsedPlanejamentoStep['zones'] }) {
  ...
  return (
    <div className="flex flex-wrap justify-center gap-1.5">
      {zones.map((zone) => (
        <span
          key={zone}
          className={`rounded-full px-2.5 py-1 font-mono text-[10px] font-black ring-1 ${
            zone === 'trap_oral'
              ? 'bg-amber-100 text-amber-900 ring-amber-200/80'
              : 'bg-pink-100 text-pink-900 ring-pink-200/80'
          }`}
        >
...
function StepBadge({ step }: { step: ParsedPlanejamentoStep }) {
  const styles: Record<ParsedPlanejamentoStep['kind'], string> = {
    anchor_category: 'bg-sky-100 text-sky-900',
...
export function LogicFlowMulherPlanejamentoTapFlow({
  ...
  const parsedSteps = useMemo(
    () => normalizedSteps.map((step, index) => parseMulherPlanejamentoStep(step, index)),
    [normalizedSteps],
  );
      ...
      <div className="relative z-10 flex min-h-0 flex-1 flex-col gap-3">
        <div className="flex items-center justify-center gap-2">
          ...
          <span className="font-mono text-[10px] font-bold uppercase tracking-widest text-pink-200">
            VF contracepção — toque para avançar
          </span>
        </div>
