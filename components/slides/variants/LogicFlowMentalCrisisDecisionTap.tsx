...
import {
  ...
  parseMentalCrisisStep,
  type ParsedMentalCrisisStep,
...
function StepBadge({ step }: { step: ParsedMentalCrisisStep }) {
  const styles: Record<ParsedMentalCrisisStep['kind'], string> = {
    anchor: 'bg-violet-100 text-violet-900',
    eliminate: 'bg-orange-100 text-orange-900',
    prioritize: 'bg-emerald-100 text-emerald-800',
    fixation: 'bg-sky-100 text-sky-900',
    step: 'bg-slate-100 text-slate-700',
  };
  const label =
    step.kind === 'eliminate'
      ? 'Eliminar'
      : step.kind === 'prioritize'
        ? 'Priorizar'
        : step.kind === 'fixation'
          ? 'Gabarito'
          : step.kind;
...
export function LogicFlowMentalCrisisDecisionTap({
  ...
  const parsed = useMemo(() => normalizedSteps.map((text) => parseMentalCrisisStep(text)), [normalizedSteps]);
  ...
  return (
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 mb-2 flex items-center justify-center gap-2 text-violet-800">
        <Heart className="h-4 w-4" />
        <span className="font-mono text-[10px] font-bold uppercase tracking-wide">Escada de cuidado</span>
      </div>
          ...
          return (
            <motion.div
              ...
              className={`overflow-hidden rounded-[1.25rem] border border-violet-200/80 bg-white/95 p-4 shadow-sm ${
                active ? 'cursor-pointer ring-2 ring-emerald-300/50' : ''
              }`}
            >
                  ...
                  {step.steps.length > 0 ? (
                    <div className="mb-2 flex flex-wrap gap-1.5">
                      {step.steps.map((s) => (
                        <span
                          key={s}
                          className="rounded-full bg-emerald-100 px-2.5 py-1 font-mono text-[10px] font-black text-emerald-900"
                        >
                          {mentalCrisisStepLabel(s)}
                        </span>
                      ))}
                    </div>
                  ) : null}
                  <p className="text-sm leading-relaxed text-slate-800">{step.text}</p>
                </>
              ) : null}
            </motion.div>
          );
      ...
      {showTapHint ? (
        <button
          type="button"
          onClick={advanceStep}
          className="relative z-10 mx-auto mt-4 flex items-center gap-2 rounded-full bg-emerald-500 px-4 py-2 text-sm font-bold text-white shadow-md"
        >
