...
function NodeRail({
  ...
  trapNodes: MentalRapsNode[];
  correctNodes: MentalRapsNode[];
  revealed: boolean;
}) {
  return (
    <div
    ...
    >
      {MENTAL_RAPS_NODES.map((node) => {
        const isTrap = trapNodes.includes(node);
        const isCorrect = correctNodes.includes(node);
...
interface DangerZoneMentalRapsTrapArenaProps {
  content: string;
...
export function DangerZoneMentalRapsTrapArena({
  content,
  items = [],
  theme,
  footerRule,
  compareRevealMode,
}: DangerZoneMentalRapsTrapArenaProps) {
    ...
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-3 md:p-4">
      ...
      <div className="relative z-10 flex flex-col gap-3">
        ...
        {items.map((item, index) => {
          ...
          const { trapNodes, correctNodes, hasRail } = inferMentalRapsTrapNodes(label, trapText, correctText);
          ...
          return (
            ...
            >
              <div className="flex flex-col gap-3 p-4">
                ...
                {hasRail ? (
                  <NodeRail trapNodes={trapNodes} correctNodes={correctNodes} revealed={isRevealed} />
                ) : null}
