...
import { Ban, Check, Pill, X } from 'lucide-react';
import type { ThemeColors } from '../core/themeGenerator';
import type { DangerZoneItem } from './DangerZone';
import {
  camExcetoTrapSlotLabel,
  inferCamExcetoTrapSlot,
  CAM_EXCETO_TRAP_SLOTS,
  type CamExcetoTrapSlot,
} from '@/lib/slides/camExcetoSlideUtils';

type TrapEntry = {
  slot: CamExcetoTrapSlot;
  index: number;
  label: string;
  detail: string;
  correct: string;
...
const SLOT_META: Record<
  CamExcetoTrapSlot,
  { label: string; tag: string; bar: string; ring: string; panel: string; text: string }
> = {
  vo_fisiologica: {
    label: 'VO + SF',
    tag: 'exceção',
  ...
  conduta_correta_distrator: {
    ...
    panel: 'from-emerald-50/95 via-white to-lime-50/90',
...
function buildEntries(items: DangerZoneItem[]): TrapEntry[] {
  return items.map((item, index) => {
    ...
    const slot = inferCamExcetoTrapSlot(label, detail, correct);
    return { slot, index, label, detail, correct };
  });
}

interface DangerZoneCamExcetoTrapArenaProps {
  content: string;
...
export function DangerZoneCamExcetoTrapArena({
  content,
  items,
  theme,
  footerRule,
}: DangerZoneCamExcetoTrapArenaProps) {
  const reduceMotion = useReducedMotion();
  const entries = useMemo(() => buildEntries(items), [items]);
  const [activeSlot, setActiveSlot] = useState<CamExcetoTrapSlot>(
    () => entries[0]?.slot ?? 'excecao_clausula',
  );
  ...
  const slotsWithItems = useMemo(() => {
    const seen = new Set<CamExcetoTrapSlot>();
    for (const e of entries) seen.add(e.slot);
    return CAM_EXCETO_TRAP_SLOTS.filter((s) => seen.has(s));
  }, [entries]);
    ...
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-4 md:p-6">
      ...
      <div className="relative z-10 mx-auto flex w-full max-w-lg flex-col gap-3.5">
        <div className="flex flex-col gap-1.5">
          <span className="inline-flex w-fit items-center gap-1.5 rounded-full border border-teal-200/80 bg-white/90 px-2.5 py-1 font-mono text-[9px] font-bold uppercase tracking-widest text-teal-900 shadow-sm">
          ...
          {slotsWithItems.map((slot) => {
            ...
            return (
              <button
                ...
                className={`min-h-11 rounded-2xl border px-2 py-2 font-mono text-[8px] font-bold uppercase transition-all ${
                  isActive
                    ? `border-2 bg-white shadow-lg ${slotMeta.ring} ring-2 ${slotMeta.text}`
                    : 'border-slate-200/90 bg-white/80 text-slate-500 shadow-sm'
                }`}
              >
                {camExcetoTrapSlotLabel(slot)}
              </button>
            );
          })}
        </div>
            ...
            >
              <div className="border-b border-black/5 px-4 py-3">
                <p className={`font-mono text-[9px] font-extrabold uppercase tracking-widest ${meta.text}`}>
                  {camExcetoTrapSlotLabel(activeSlot)} · {meta.tag}
                </p>
                <h3 className="font-body text-sm font-semibold text-slate-900">{activeEntry.label}</h3>
              </div>
              <div className="space-y-3 px-4 py-3">
                <p className="font-body text-sm text-slate-700">{activeEntry.detail}</p>
                <button
                  type="button"
                ...
                >
                  <div className="flex items-start gap-2">
                    ...
                    <p className="font-body text-sm font-semibold text-slate-900">
                      {revealed.has(activeEntry.index)
                        ? activeEntry.correct
                        : 'Revelar conduta correta desta letra'}
                    </p>
                  </div>
                </button>
              </div>
