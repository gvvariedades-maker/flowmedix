...
const ROUTES: { id: RouteSlot; label: string; tag: string }[] = [
  { id: 'iv', label: 'IV', tag: 'rápida' },
  { id: 'im', label: 'IM', tag: 'rápida' },
  { id: 'sc', label: 'SC', tag: 'lenta' },
  { id: 'vo', label: 'VO', tag: 'variável' },
];

function extractRoutes(text: string): RouteSlot[] {
  const lower = text.toLowerCase();
  const found = new Set<RouteSlot>();
  if (/intravenosa|\biv\b|imediata|rápida na corrente|alta pressão|bomba|pressuriz/.test(lower)) {
    found.add('iv');
  }
  if (/\bim\b|intramuscular|mais rápida que sc/.test(lower)) found.add('im');
  if (/subcutânea|\bsc\b|lenta e contínua|lenta|gradual|hipodérmico|volume pequeno/.test(lower)) {
    found.add('sc');
  }
  if (/\bvo\b|oral|tgi|variável/.test(lower)) found.add('vo');
  if (/absorção rápida|rapidamente/.test(lower) && !/mais lenta/.test(lower)) {
    found.add('iv');
    found.add('im');
  }
  if (/dose grande|efeito rápido/.test(lower)) {
    found.add('iv');
    found.add('im');
  }
  return [...found];
...
function inferRouteSlots(
  ...
  detail: string,
  correct: string,
): { trapRoutes: RouteSlot[]; correctRoutes: RouteSlot[]; hasRail: boolean } {
  const trapText = `${label} ${detail}`.toLowerCase();
  const correctText = correct.toLowerCase();

  if (/técnica|indicação|farmacocinética|velocidade desejada|corrente sanguínea/.test(trapText + correctText)) {
    if (/técnica, não pela indicação|responder pela técnica/.test(trapText)) {
      return { trapRoutes: [], correctRoutes: ['sc'], hasRail: false };
    }
  }

  let trapRoutes = extractRoutes(trapText);
  let correctRoutes = extractRoutes(correctText);

  if (/letra a|absorção rápida/.test(trapText)) trapRoutes = ['iv', 'im'];
  if (/letra d|dose grande/.test(trapText)) trapRoutes = ['iv', 'im'];
  if (/letra e|alta pressão/.test(trapText)) trapRoutes = ['iv'];
  if (/confundir sc com im/.test(trapText)) trapRoutes = ['im'];

  if (correctRoutes.length === 0) correctRoutes = ['sc'];

  return {
    trapRoutes,
    correctRoutes,
    hasRail: trapRoutes.length > 0,
  };
}
...
function RouteRail({
  ...
  trapRoutes: RouteSlot[];
  correctRoutes: RouteSlot[];
  revealed: boolean;
}) {
  return (
    <div className="flex items-stretch justify-between gap-1 rounded-xl border border-emerald-200/80 bg-emerald-50/50 p-1.5">
      {ROUTES.map((route) => {
        const isTrap = trapRoutes.includes(route.id);
        const isCorrect = correctRoutes.includes(route.id);
        ...
        return (
          ...
          >
            ...
            <span className="font-mono text-[7px] uppercase text-slate-500">{route.tag}</span>
          </div>
        );
      })}
    </div>
...
function RouteTrapCard({
  ...
  const { trapRoutes, correctRoutes, hasRail } = inferRouteSlots(label, trapText, correctText);
        ...
        <div className="grid grid-rows-[auto_auto_minmax(0,1fr)_auto] gap-2 p-4">
          ...
          {hasRail ? (
            ...
            <div className="flex items-center gap-2 rounded-xl border border-slate-200/80 bg-slate-50/80 px-3 py-2">
              <Syringe className="h-4 w-4 shrink-0 text-slate-600" aria-hidden />
...
interface DangerZoneRouteTrapProps {
  content: string;
...
export function DangerZoneRouteTrap({
  content,
  items,
  theme,
  footerRule,
  compareRevealMode = 'auto',
}: DangerZoneRouteTrapProps) {
    ...
    <div className="relative flex min-h-0 w-full min-w-0 flex-1 flex-col overflow-y-auto p-4 md:p-6">
      ...
      <div className="relative z-10 flex flex-col">
        ...
        <p className="mb-4 flex justify-center">
          ...
          >
            <span className={`font-body ${theme.textSecondary}`}>Vias corrigidas:</span>
        ...
        {allRevealed ? (
          ...
          >
            <span className="font-mono text-xs font-bold uppercase tracking-widest text-emerald-900">
              {isTapMode ? 'SC dominada — lenta e contínua, não rápida' : 'Revise IV · IM · SC · VO antes da prova'}
            </span>
          </motion.div>
        ) : null}
      </div>
