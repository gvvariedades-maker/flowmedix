    ...
    >
      <div className="login-auth-card">
        <div className="flex items-start gap-4 border-b border-slate-200 pb-5">
          <div
            className="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-[var(--color-success-dim)]"
            aria-hidden
          >
            <BookMarked className="h-7 w-7 text-[var(--color-success-text)]" strokeWidth={1.5} />
          </div>
          <div>
...
function WizardNovoCadernoForm({
  ...
  // Seleção do hub vive em sessionStorage; lida uma vez, no cliente.
  ...
  // Seleção perdida (storage limpo, aba nova) ≠ seleção vazia proposital.
  const selecaoPerdida = modoDesempenho && selecaoDesempenho !== null && selecaoDesempenho.length === 0;
  const semQuestoesNaSelecao =
    modoDesempenho && !selecaoPerdida && (selecaoDesempenho?.length ?? 0) > 0 && batchPreview.length === 0;
  ...
  const handleCreateWithBatch = async () => {
    ...
    // Defesa em profundidade: disabled no CTA não basta — o handler também recusa.
    ...
    if (semQuestoesNaSelecao) {
      // Nunca completar o lote com assunto que o aluno não escolheu.
        ...
        <div className="mb-6 space-y-4 border-b border-slate-200 pb-5">
          <div className="flex items-start gap-4">
            <div
              ...
              aria-hidden
            >
              <Sparkles className="h-7 w-7 text-[var(--color-success-text)]" strokeWidth={1.5} />
            </div>
          ...
          {step === 1 ? (
                  ...
                  {titleSuggestions.map((suggestion) => (
                    <button
                      ...
                      className={cn(
                        ...
                        title === suggestion
                          ? 'border-[var(--color-success)]/45 bg-[var(--color-success-dim)] text-[var(--color-success-text)]'
                          : 'border-slate-200 bg-white text-slate-600 hover:border-[var(--color-success)]/35 hover:text-slate-900',
                      )}
                    >
                      {suggestion}
                    </button>
            ...
            >
              {selecaoPerdida ? (
                ...
                <PresetPreview
                  preset={preset}
                  batchCount={batchPreview.length}
                  carregandoSelecao={modoDesempenho && selecaoDesempenho === null}
                />
              )}
              ...
              <div className="flex flex-col gap-3 pt-2 sm:flex-row sm:justify-between">
                ...
                <button
                  ...
                  disabled={
                    loading ||
                    (modoDesempenho && selecaoDesempenho === null) ||
                    selecaoPerdida ||
    ...
    <div className="space-y-4 rounded-2xl border border-slate-200 bg-slate-50 p-4">
      {estrito ? (
        <p className="text-sm font-bold text-slate-900">
          Assuntos escolhidos no seu desempenho
        </p>
      ) : preset.banca ? (
        <p className="text-sm font-bold text-slate-900">
