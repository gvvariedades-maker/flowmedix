      ...
      {itens.map((item, index) => (
        <div key={item} className="flex items-center gap-2">
          ...
          {index < itens.length - 1 ? (
            <ChevronRight className="hidden h-4 w-4 text-[#3d6b0f] md:block" aria-hidden />
          ) : null}
        </div>
      ))}
    </div>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        {etapas.map((etapa) => (
          <article key={etapa.nome} className={`p-4 ${AJUDA_SURFACE_SM}`}>
            <span className="flex h-9 w-9 items-center justify-center rounded-full bg-[#3d6b0f] text-sm font-black text-white">{etapa.n}</span>
      ...
      <ol className="mt-4 space-y-3">
        {itens.map((item, index) => (
          <li key={item} className="flex gap-3 text-sm font-semibold leading-relaxed text-slate-700">
            <span className="flex h-6 w-6 shrink-0 items-center justify-center rounded-full bg-[#3d6b0f] text-xs font-black text-white">
              {index + 1}
            </span>
            <span>{item}</span>
          </li>
      ...
      <div className="mt-5 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
        {itens.map((item) => (
          <article key={item.nome} className={`p-4 ${AJUDA_SURFACE_SM}`}>
            <BookOpenCheck className="h-5 w-5 text-[#3d6b0f]" aria-hidden />
            <h3 className="mt-3 text-lg font-black text-slate-900">{item.nome}</h3>
            <p className="mt-2 text-sm leading-relaxed text-slate-600">{item.desc}</p>
            <p className="mt-3 text-xs font-bold leading-relaxed text-[#3d6b0f]">No AVANT: {item.noAvant}</p>
      ...
      <div className="mt-5 space-y-3">
        {itens.map((item) => (
          <details key={item.q} className="group rounded-2xl border border-slate-200 bg-white p-4">
            <summary className="cursor-pointer text-sm font-black text-slate-900 [&::-webkit-details-marker]:text-[#3d6b0f]">
              {item.q}
            </summary>
      ...
      <ul className="mt-4 space-y-2 text-xs leading-relaxed text-slate-600">
        {itens.map((item) => (
          <li key={item} className="flex gap-2">
            <span className="mt-1 h-1.5 w-1.5 shrink-0 rounded-full bg-[#8fe020]" aria-hidden />
            <span>{item}</span>
          </li>
        ))}
      </ul>
...
export function CtaFinal({ titulo, subtitulo, primary, secondary }: { titulo: string; subtitulo: string; primary: Cta; secondary: Cta }) {
  return (
    <section className="rounded-3xl border border-[rgba(143,224,32,0.35)] bg-gradient-to-br from-[rgba(143,224,32,0.12)] to-white p-5 shadow-sm md:p-7">
