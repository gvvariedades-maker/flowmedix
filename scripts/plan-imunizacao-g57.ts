#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g57 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g57 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
 *   npm run plan:imunizacao-g57
 *   npm run catalog:export-lote -- --lote=imunizacao-g57 --from-manifest=data/catalog-migration/imunizacao-g57/manifest.json
 */
...
const PRIOR_P1_LOTES = ['imunizacao-g56'];
...
/**
 * Exclui applied + handcraft_ready (meta) + repair + prior P1 lotes (g56).
 * NÃO usa imunizacao-exclude-done (lista legado/híbrido quase total — ver g08).
 */
function loadExcludeSlugs(skipLote?: string): Set<string> {
  ...
  for (const prior of PRIOR_P1_LOTES) {
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g57';
  ...
  const slugsFile = resolve(loteDir, 'g57-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43 superseded. g57 = 2º lote P1 (após g56). Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g57',
      'npm run catalog:export-lote -- --lote=imunizacao-g57 --from-manifest=data/catalog-migration/imunizacao-g57/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g57 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g57 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g57',
      'npm run validate:goldens -- --strict --lote=imunizacao-g57',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g57] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g57] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g57] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g57] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
