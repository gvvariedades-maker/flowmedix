#!/usr/bin/env tsx
/**
 * Escreve artifacts/neurocanvas-audit-report.md (relatório consolidado).
 * Somente geração de documentação — não altera runtime do player.
 */
...
const resolver = JSON.parse(
  readFileSync('artifacts/neurocanvas-resolver-audit-catalog-full.json', 'utf8'),
);
...
const out = resolve('artifacts/neurocanvas-audit-report.md');
mkdirSync(resolve('artifacts'), { recursive: true });
writeFileSync(out, md, 'utf8');
console.log('Wrote', out, 'bytes', Buffer.byteLength(md, 'utf8'));
