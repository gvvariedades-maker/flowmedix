#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g61 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g61 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
 *   npm run plan:imunizacao-g61
 *   npm run catalog:export-lote -- --lote=imunizacao-g61 --from-manifest=data/catalog-migration/imunizacao-g61/manifest.json
 */
...
/** Exclui applied + handcraft_ready + superseded (handcraft-meta) + exclude-done + applied artifacts. */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g61';
  ...
  const slugsFile = resolve(loteDir, 'g61-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43/g44 superseded. g61 exclusivamente P1/P2 pós g60. Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g61',
      'npm run catalog:export-lote -- --lote=imunizacao-g61 --from-manifest=data/catalog-migration/imunizacao-g61/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g61 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g61 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g61',
      'npm run validate:goldens -- --strict --lote=imunizacao-g61',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g61] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g61] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g61] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g61] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
