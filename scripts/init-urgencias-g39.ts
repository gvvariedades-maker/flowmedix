#!/usr/bin/env tsx
/**
 * Inicializa urgencias-g39 — 8 slugs · 10º lote urgencias_generico (cauda cluster).
 * Slugs selecionados por: npx tsx scripts/plan-urgencias-g39.ts
 */
...
function main() {
  ...
  if (!existsSync(planManifest)) {
    console.error(`[init:urgencias-g39] Rode antes: npx tsx scripts/plan-urgencias-g39.ts`);
    process.exit(1);
  }
  const { slugs } = JSON.parse(readFileSync(planManifest, 'utf8')) as { slugs: string[] };
  if (slugs.length !== 8) {
    console.error(`[init:urgencias-g39] manifest deve ter 8 slugs, tem ${slugs.length}`);
    process.exit(1);
  }
  ...
  for (const slug of slugs) {
    ...
    if (!existsSync(src)) {
      console.error(`[init:urgencias-g39] MISSING ${src}`);
      process.exit(1);
    }
    copyFileSync(src, dest);
    console.log(`[init:urgencias-g39] copied ${slug}`);
  }
    ...
    `${JSON.stringify(
      {
        ...
        filters: {
          ...
          batch: 'g39-generico-tail-batch-8',
        },
        slugs,
      },
      null,
    ...
    `${JSON.stringify(
      {
        ...
        notes:
          'g39 — 10º lote cauda generico · peçonhentos C/E + XABCDE/evisceração generico · escorpionismo cardíaco → choque',
      },
      null,
      2,
    )}\n`,
  ...
  console.log(`[init:urgencias-g39] manifest + lote-meta OK (${slugs.length} slugs)`);
}
