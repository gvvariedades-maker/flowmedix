#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g54 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g54
 *   npm run catalog:export-lote -- --lote=imunizacao-g54 --from-manifest=data/catalog-migration/imunizacao-g54/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g54';
  ...
  const slugsFile = resolve(loteDir, 'g54-slugs.json');
  ...
  const manifest = {
    ...
    filters: {
      ...
      mix: 'calendário residual + técnica/conceito P0-adjacent',
    },
    slugs: picked,
  };
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — P0 calendário residual (2) + fallback técnica/conceito (6). Sem overlap g01–g53. Handcraft golden-v1. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g54',
      'npm run catalog:export-lote -- --lote=imunizacao-g54 --from-manifest=data/catalog-migration/imunizacao-g54/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g54 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g54 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g54',
      'npm run validate:goldens -- --strict --lote=imunizacao-g54',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g54] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g54] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g54] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
