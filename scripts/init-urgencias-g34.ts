#!/usr/bin/env tsx
/**
 * Inicializa urgencias-g34 — 8 slugs · 5º lote urgencias_generico (cauda cluster).
 * Slugs selecionados por: npx tsx scripts/plan-urgencias-g34.ts
 */
...
function main() {
  ...
  if (!existsSync(planManifest)) {
    console.error(`[init:urgencias-g34] Rode antes: npx tsx scripts/plan-urgencias-g34.ts`);
    process.exit(1);
  }
  const { slugs } = JSON.parse(readFileSync(planManifest, 'utf8')) as { slugs: string[] };
  if (slugs.length !== 8) {
    console.error(`[init:urgencias-g34] manifest deve ter 8 slugs, tem ${slugs.length}`);
    process.exit(1);
  }
  ...
  for (const slug of slugs) {
    ...
    if (!existsSync(src)) {
      console.error(`[init:urgencias-g34] MISSING ${src}`);
      process.exit(1);
    }
    copyFileSync(src, dest);
    console.log(`[init:urgencias-g34] copied ${slug}`);
  }
    ...
    `${JSON.stringify(
      {
        ...
        filters: {
          ...
          batch: 'g34-generico-tail-batch-5',
        },
        slugs,
      },
      null,
    ...
    `${JSON.stringify(
      {
        ...
        notes: 'g34 — 5º lote cauda urgencias_generico · inferência por enunciado (g30–g33 pattern)',
      },
      null,
      2,
    )}\n`,
  ...
  console.log(`[init:urgencias-g34] manifest + lote-meta OK (${slugs.length} slugs)`);
}
