#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g60 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g60 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
 *   npm run plan:imunizacao-g60
 *   npm run catalog:export-lote -- --lote=imunizacao-g60 --from-manifest=data/catalog-migration/imunizacao-g60/manifest.json
 */
...
/**
 * Exclui applied + handcraft_ready + repair aplicados.
 * NÃO usa imunizacao-exclude-done (lista legado/híbrido quase total).
 * Absorve lotes P1 irmãos g56–g59 quando já planejados.
 */
function loadExcludeSlugs(skipLote?: string): Set<string> {
  ...
  for (const prior of ['imunizacao-g56', 'imunizacao-g57', 'imunizacao-g58', 'imunizacao-g59']) {
    absorbManifest(prior);
  }
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g60';
  ...
  const slugsFile = resolve(loteDir, 'g60-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43/g44 superseded. g60 exclusivamente P1/P2. Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g60',
      'npm run catalog:export-lote -- --lote=imunizacao-g60 --from-manifest=data/catalog-migration/imunizacao-g60/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g60 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g60 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g60',
      'npm run validate:goldens -- --strict --lote=imunizacao-g60',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g60] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g60] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g60] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g60] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
