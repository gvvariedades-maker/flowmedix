#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g70 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g70 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g70';
  ...
  const slugsFile = resolve(loteDir, 'g70-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43/g44/g49 superseded. g70 exclusivamente P1/P2 pós g69. exclude-done omitido. Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g70',
      'npm run catalog:export-lote -- --lote=imunizacao-g70 --from-manifest=data/catalog-migration/imunizacao-g70/manifest.json',
      ...
      'npm run audit:slug-alignment -- --lote=imunizacao-g70 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g70',
  ...
  console.log(`[plan:imunizacao-g70] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g70] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g70] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g70] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
