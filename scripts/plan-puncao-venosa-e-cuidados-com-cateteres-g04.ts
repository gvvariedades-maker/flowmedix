#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g04 (P0 puncao_tempo).
 *
...
const P0_CLUSTER = 'Tempo / observação pós-procedimento';
...
function main(): void {
  ...
  if (picked.length < batchSize) {
    throw new Error(
      `Só ${picked.length} slugs P0 tempo disponíveis (pool=${pool.length}, excluídos=${exclude.size})`,
    );
  }
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 — puncao_tempo (Tempo / intervalos / observação)',
    slug_count: picked.length,
    pedagogical_branch_target: BRANCH,
    anchors: ['examples/questao-premium-cpcon-puncao-troca-equipos-intervalos.json'],
    anchor_slug: 'cpcon-uepb-enfermagem-puncao-venosa-e-cuidados-com-cateteres-1779340232037-0',
    workflow: [
      'npm run plan:puncao-venosa-e-cuidados-com-cateteres-g04',
  ...
  console.log(`[plan:puncao-g04] lote=${LOTE} slugs=${picked.length} pool=${pool.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:puncao-g04] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
