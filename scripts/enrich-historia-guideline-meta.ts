#!/usr/bin/env tsx
/**
 * Preenche meta.sources tier A + content_review.guideline_snapshot (História).
 *
...
import { enrichHistoriaGuidelineMeta } from '@/lib/catalogMigration/historiaPedagogy';
...
function main(): void {
  ...
  for (const { path, slug } of targets) {
    ...
    report.scanned += 1;
    const { payload: next, changed, reasons } = enrichHistoriaGuidelineMeta(payload);
    if (changed) {
      report.enriched += 1;
      if (write) {
  ...
  const outPath = resolve(outDir, 'historia-guideline-meta-enrich.json');
