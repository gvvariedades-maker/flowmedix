#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g72 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g72 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
 *   npm run plan:imunizacao-g72
 *   npm run catalog:export-lote -- --lote=imunizacao-g72 --from-manifest=data/catalog-migration/imunizacao-g72/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g72';
  ...
  const slugsFile = resolve(loteDir, 'g72-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43/g44/g49 superseded. g51 exported only — não duplicar slugs. g72 exclusivamente P1/P2 pós g71. Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g72',
      'npm run catalog:export-lote -- --lote=imunizacao-g72 --from-manifest=data/catalog-migration/imunizacao-g72/manifest.json',
  ...
  console.log(`[plan:imunizacao-g72] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g72] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g72] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g72] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
