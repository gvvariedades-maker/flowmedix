#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g26 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g26
 *   npm run catalog:export-lote -- --lote=imunizacao-g26 --from-manifest=data/catalog-migration/imunizacao-g26/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g26';
  ...
  const slugsFile = resolve(loteDir, 'g26-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — slugs sem overlap com g01–g25 (incl. g11/g14–g25 handcraft_ready). Handcraft golden-v1 calendário. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g26',
      'npm run catalog:export-lote -- --lote=imunizacao-g26 --from-manifest=data/catalog-migration/imunizacao-g26/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g26 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g26 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g26',
      'npm run validate:goldens -- --strict --lote=imunizacao-g26',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g26] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g26] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g26] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
