#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g71 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g71 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
 *   npm run plan:imunizacao-g71
 *   npm run catalog:export-lote -- --lote=imunizacao-g71 --from-manifest=data/catalog-migration/imunizacao-g71/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g71';
  ...
  const slugsFile = resolve(loteDir, 'g71-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43/g44/g49 superseded. g51 exported (reservado repair). g71 exclusivamente P1/P2 pós g70. Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g71',
      'npm run catalog:export-lote -- --lote=imunizacao-g71 --from-manifest=data/catalog-migration/imunizacao-g71/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g71 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g71 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g71',
      'npm run validate:goldens -- --strict --lote=imunizacao-g71',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g71] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g71] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g71] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g71] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
