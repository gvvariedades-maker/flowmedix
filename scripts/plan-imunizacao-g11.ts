#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g11 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g11
 *   npm run catalog:export-lote -- --lote=imunizacao-g11 --from-manifest=data/catalog-migration/imunizacao-g11/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g11';
  ...
  const slugsFile = resolve(loteDir, 'g11-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Replanejado 2026-07-03 — slugs sem overlap com g08/g10/g12/g13 aplicados. Handcraft golden-v1 calendário. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g11',
      'npm run catalog:export-lote -- --lote=imunizacao-g11 --from-manifest=data/catalog-migration/imunizacao-g11/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g11 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g11 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g11',
      'npm run capture:questao-review -- --lote=imunizacao-g11',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g11] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g11] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g11] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
