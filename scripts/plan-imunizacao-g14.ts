#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g14 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g14
 *   npm run catalog:export-lote -- --lote=imunizacao-g14 --from-manifest=data/catalog-migration/imunizacao-g14/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g14';
  ...
  const slugsFile = resolve(loteDir, 'g14-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Replanejado 2026-07-03 — slugs sem overlap com g08–g13 (incl. g11 replanejado). Handcraft golden-v1 calendário. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g14',
      'npm run catalog:export-lote -- --lote=imunizacao-g14 --from-manifest=data/catalog-migration/imunizacao-g14/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g14 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g14 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g14',
      'npm run capture:questao-review -- --lote=imunizacao-g14',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g14] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g14] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g14] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
