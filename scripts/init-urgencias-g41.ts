#!/usr/bin/env tsx
/**
 * Inicializa urgencias-g41 — 8 slugs · 12º lote urgencias_generico (cauda cluster).
 * Slugs selecionados por: npx tsx scripts/plan-urgencias-g41.ts
 */
...
function main() {
  ...
  if (!existsSync(planManifest)) {
    console.error(`[init:urgencias-g41] Rode antes: npx tsx scripts/plan-urgencias-g41.ts`);
    process.exit(1);
  }
  const { slugs } = JSON.parse(readFileSync(planManifest, 'utf8')) as { slugs: string[] };
  if (slugs.length !== 8) {
    console.error(`[init:urgencias-g41] manifest deve ter 8 slugs, tem ${slugs.length}`);
    process.exit(1);
  }
  ...
  for (const slug of slugs) {
    ...
    if (!existsSync(src)) {
      console.error(`[init:urgencias-g41] MISSING ${src}`);
      process.exit(1);
    }
    copyFileSync(src, dest);
    console.log(`[init:urgencias-g41] copied ${slug}`);
  }
    ...
    `${JSON.stringify(
      {
        ...
        filters: {
          ...
          batch: 'g41-generico-tail-batch-8',
        },
        slugs,
      },
      null,
    ...
    `${JSON.stringify(
      {
        ...
        notes:
          'g41 — 12º lote cauda generico · coluna cervical C/E · APH/PS ESF · V/F primeiros socorros · picada abelha · Access urgência/emergência · SAMU CRMU EXCETO',
      },
      null,
      2,
    )}\n`,
  ...
  console.log(`[init:urgencias-g41] manifest + lote-meta OK (${slugs.length} slugs)`);
}
