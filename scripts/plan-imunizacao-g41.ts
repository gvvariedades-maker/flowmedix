#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g41 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g41
 *   npm run catalog:export-lote -- --lote=imunizacao-g41 --from-manifest=data/catalog-migration/imunizacao-g41/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g41';
  ...
  const slugsFile = resolve(loteDir, 'g41-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — slugs sem overlap com g01–g40 (incl. handcraft_ready/planned). Handcraft golden-v1 calendário. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g41',
      'npm run catalog:export-lote -- --lote=imunizacao-g41 --from-manifest=data/catalog-migration/imunizacao-g41/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g41 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g41 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g41',
      'npm run validate:goldens -- --strict --lote=imunizacao-g41',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g41] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g41] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g41] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
