#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g59 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g59 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
 *   npm run plan:imunizacao-g59
 *   npm run catalog:export-lote -- --lote=imunizacao-g59 --from-manifest=data/catalog-migration/imunizacao-g59/manifest.json
 */
...
/** Exclui applied + handcraft_ready + repair + P1 lotes g51/g56–g58. Sem exclude-done. */
function loadExcludeSlugs(skipLote?: string): Set<string> {
  const exclude = new Set<string>();
  const migrationRoot = resolve(process.cwd(), 'data/catalog-migration');

  const absorbManifest = (loteName: string): void => {
    if (skipLote && loteName === skipLote) return;
  ...
  for (const p1Lote of [
    ...
    absorbManifest(p1Lote);
  }

  const repairLoteRe = /^imunizacao-(exceto-[\w-]+|[\w-]+-repair)$/;
  for (const name of readdirSync(migrationRoot)) {
    if (!repairLoteRe.test(name)) continue;
    absorbManifest(name);
  }
...
function isCatalogImunizacaoSlug(slug: string, completoSlugs: Set<string>): boolean {
  ...
  return /-(enfermagem-imunizacao|geral-imunizacao)-/.test(slug) || /-imunizacao-/.test(slug);
}
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g59';
  ...
  const slugsFile = resolve(loteDir, 'g59-slugs.json');
  ...
  const manifest = {
    ...
    filters: {
      ...
      exclude_done_not_used: true,
      p1_predecessors: ['imunizacao-g51', 'imunizacao-g56', 'imunizacao-g57', 'imunizacao-g58'],
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43/g44 superseded. g59 P1/P2 após g58. exclude-done omitido. Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g59',
      'npm run catalog:export-lote -- --lote=imunizacao-g59 --from-manifest=data/catalog-migration/imunizacao-g59/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g59 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g59 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g59',
      'npm run validate:goldens -- --strict --lote=imunizacao-g59',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g59] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g59] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g59] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g59] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
