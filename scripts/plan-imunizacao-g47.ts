#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g47 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g47
 *   npm run catalog:export-lote -- --lote=imunizacao-g47 --from-manifest=data/catalog-migration/imunizacao-g47/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g47';
  ...
  // Tail P0: cluster calendário esgotado — backfill Default imunização
  ...
  const slugsFile = resolve(loteDir, 'g47-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — tail P0 calendário: infantil residual + Default imunização (backfill pós-esgotamento g01–g46). Handcraft golden-v1 calendário. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g47',
      'npm run catalog:export-lote -- --lote=imunizacao-g47 --from-manifest=data/catalog-migration/imunizacao-g47/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g47 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g47 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g47',
      'npm run validate:goldens -- --strict --lote=imunizacao-g47',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g47] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g47] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g47] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
