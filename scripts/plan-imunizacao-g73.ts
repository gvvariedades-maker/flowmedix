#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g73 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g73 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
 *   npm run plan:imunizacao-g73
 *   npm run catalog:export-lote -- --lote=imunizacao-g73 --from-manifest=data/catalog-migration/imunizacao-g73/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g73';
  ...
  const slugsFile = resolve(loteDir, 'g73-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43/g44/g49 superseded. g51 superseded por g56 — não duplicar. g73 exclusivamente P1/P2 pós g72. Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g73',
      'npm run catalog:export-lote -- --lote=imunizacao-g73 --from-manifest=data/catalog-migration/imunizacao-g73/manifest.json',
  ...
  console.log(`[plan:imunizacao-g73] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g73] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g73] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g73] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
