#!/usr/bin/env tsx
/**
 * Preenche meta.sources + content_review.guideline_snapshot (ADME) em questões Farmacodinâmica.
 *
 * Uso:
...
import { enrichFarmacoGuidelineMeta } from '@/lib/catalogMigration/farmacoPedagogy';
...
function main(): void {
  ...
  for (const { path, slug } of targets) {
    ...
    report.scanned += 1;
    const { payload: next, changed, reasons } = enrichFarmacoGuidelineMeta(payload);
    if (changed) {
      report.enriched += 1;
      if (write) {
  ...
  const outPath = resolve(outDir, 'farmacodinamica-guideline-meta-enrich.json');
