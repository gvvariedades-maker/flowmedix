#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g56 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g56 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
 *   npm run plan:imunizacao-g56
 *   npm run catalog:export-lote -- --lote=imunizacao-g56 --from-manifest=data/catalog-migration/imunizacao-g56/manifest.json
 */
...
/**
 * Exclui applied g01–g13 + handcraft_ready + repair aplicados.
 * NÃO usa imunizacao-exclude-done (lista legado/híbrido quase total — ver g08).
 * Não bloqueia lotes só `planned` (g44–g55 calendário fora de ordem).
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g56';
  ...
  const slugsFile = resolve(loteDir, 'g56-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43 superseded. g44–g55 planned calendário fora de ordem (overlap possível). g56 = 1º lote P1. Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g56',
      'npm run catalog:export-lote -- --lote=imunizacao-g56 --from-manifest=data/catalog-migration/imunizacao-g56/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g56 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g56 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g56',
      'npm run validate:goldens -- --strict --lote=imunizacao-g56',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g56] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g56] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g56] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g56] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
