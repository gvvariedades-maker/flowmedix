#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g49 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g49
 *   npm run catalog:export-lote -- --lote=imunizacao-g49 --from-manifest=data/catalog-migration/imunizacao-g49/manifest.json
 */
...
const SKIP_FROM_EXCLUDE = new Set([
  'imunizacao-g42', // meta handcraft_ready sem questions/ — slugs reatribuídos ao g49
  'imunizacao-g43', // duplicata de g42
  'imunizacao-g44', // duplicata de g42
  'imunizacao-g45', // cluster Default — fora do P0 calendário
  'imunizacao-g48', // cluster Default — fora do P0 calendário
]);
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g49';
  ...
  const slugsFile = resolve(loteDir, 'g49-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — slugs reatribuídos de g42–g48 (duplicatas/Default sem handcraft). 2 infantil Fundatec + 6 adulto Vunesp. Handcraft golden-v1 calendário.',
    workflow: [
      'npm run plan:imunizacao-g49',
      'npm run catalog:export-lote -- --lote=imunizacao-g49 --from-manifest=data/catalog-migration/imunizacao-g49/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g49 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g49 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g49',
      'npm run validate:goldens -- --strict --lote=imunizacao-g49',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g49] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g49] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g49] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
