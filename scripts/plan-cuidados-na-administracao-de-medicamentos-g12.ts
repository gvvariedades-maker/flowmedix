#!/usr/bin/env tsx
/**
 * Seleciona slugs para cuidados-na-administracao-de-medicamentos-g12 (cauda pós-g11).
 *
 *   npm run plan:cuidados-na-administracao-de-medicamentos-g12
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'cuidados-na-administracao-de-medicamentos-g12';
    ...
    JSON.stringify(
      {
        ...
        status: 'planned',
        priority: 'P1 — cauda cam_generico (lote 3/5)',
  ...
  console.log(`[plan:cuidados-g12] lote=${lote} slugs=${picked.length} pool_remaining=${pool.length}`);
