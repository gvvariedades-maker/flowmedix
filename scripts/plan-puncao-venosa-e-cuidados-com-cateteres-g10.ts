#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g10 (P0 puncao_flebite — continuação g01).
 *
...
function main(): void {
  ...
  if (picked.length === 0) {
    throw new Error(`Nenhum slug flebite disponível (pool=${pool.length}, excluídos=${exclude.size})`);
  }
  ...
  const manifest = {
    ...
    filters: {
      ...
      note: picked.length < requested ? `cluster flebite tem ${pool.length} slugs restantes` : undefined,
    },
    slugs: picked,
  };
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 — puncao_flebite (continuação g01)',
    slug_count: picked.length,
    pedagogical_branch_target: BRANCH,
    anchors: ['examples/questao-premium-avancasp-puncao-infiltracao-flebite.json'],
    anchor_slug: 'avancasp-enfermagem-puncao-venosa-e-cuidados-com-cateteres-1779340254185-7',
    workflow: [
      'npm run plan:puncao-venosa-e-cuidados-com-cateteres-g10',
  ...
  console.log(`[plan:puncao-g10] lote=${LOTE} slugs=${picked.length} pool=${pool.length} remaining=${pool.length - picked.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:puncao-g10] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
