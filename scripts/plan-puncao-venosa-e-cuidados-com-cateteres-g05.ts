#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g05 (P0 puncao_periferica_antissepsia).
 *
...
const P0_CLUSTER = 'Punção venosa periférica';
...
function main(): void {
  ...
  if (picked.length < batchSize) {
    throw new Error(
      `Só ${picked.length} slugs P0 periférica disponíveis (pool=${pool.length}, excluídos=${exclude.size})`,
    );
  }
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 — puncao_periferica_antissepsia (Punção periférica / antissepsia)',
  ...
  console.log(`[plan:puncao-g05] lote=${LOTE} slugs=${picked.length} pool=${pool.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:puncao-g05] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
