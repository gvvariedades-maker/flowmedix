#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g13 (P0 Técnica de punção periférica — 7 slugs).
 *
...
const P0_CLUSTER = 'Técnica de punção periférica';
...
function main(): void {
  ...
  if (picked.length === 0) {
    throw new Error(`Nenhum slug técnica periférica disponível (pool=${pool.length}, excluídos=${exclude.size})`);
  }
  ...
  const manifest = {
    ...
    filters: {
      ...
      note: pool.length - picked.length === 0 ? 'cluster técnica periférica esgotado' : undefined,
    },
    slugs: picked,
  };
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 — puncao_periferica_antissepsia (Técnica de punção periférica)',
    slug_count: picked.length,
    pedagogical_branch_target: BRANCH,
    anchors: ['examples/questao-premium-funpar-puncao-tecnica-periferica.json'],
    anchor_slug: 'nc-ufpr-funpar-enfermagem-puncao-venosa-e-cuidados-com-cateteres-1779340254185-4',
    workflow: [
      'npm run plan:puncao-venosa-e-cuidados-com-cateteres-g13',
  ...
  console.log(`[plan:puncao-g13] lote=${LOTE} slugs=${picked.length} pool=${pool.length} remaining=${pool.length - picked.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:puncao-g13] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
