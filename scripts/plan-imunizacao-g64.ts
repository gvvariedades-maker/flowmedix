#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g64 (P1 pós-calendário) a partir do cluster report.
 * Pool P0 calendário esgotado em g42 — g64 usa cadeia de frio, EXCETO, V/F intervalos e genérico.
 *
 *   npm run plan:imunizacao-g64
 *   npm run catalog:export-lote -- --lote=imunizacao-g64 --from-manifest=data/catalog-migration/imunizacao-g64/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g64';
  ...
  const slugsFile = resolve(loteDir, 'g64-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — pool P0 calendário esgotado (g42). g43/g44 superseded. g64 exclusivamente P1/P2 pós g63. Evitar palavra “termo” isolada (false trigger drift).',
    workflow: [
      'npm run plan:imunizacao-g64',
      'npm run catalog:export-lote -- --lote=imunizacao-g64 --from-manifest=data/catalog-migration/imunizacao-g64/manifest.json',
  ...
  console.log(`[plan:imunizacao-g64] lote=${lote} slugs=${picked.length}`);
  console.log(
    `[plan:imunizacao-g64] pools: cadeia=${cadeiaFrio.length} exceto=${exceto.length} vf=${vfIntervalos.length} gen=${generico.length}`,
  );
  console.log(`[plan:imunizacao-g64] slugs_file=${slugsFile}`);
  for (const s of picked) {
    console.log(`  · ${s} [${clusterBySlug.get(s) ?? '?'}]`);
  }
  console.log(
    `[plan:imunizacao-g64] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
