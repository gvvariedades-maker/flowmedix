#!/usr/bin/env tsx
/**
 * Seleciona 8 slugs para urgencias-g46 — 17º lote cauda urgencias_generico.
 *
 *   npx tsx scripts/plan-urgencias-g46.ts
 */
...
function main() {
  ...
  if (picked.length < LIMIT) {
    console.error(`[plan:urgencias-g46] pool insuficiente: ${picked.length}/${LIMIT}`);
    process.exit(1);
  }
  ...
  console.log(`[plan:urgencias-g46] pool=${pool.length} picked=${picked.length}`);
  for (const p of picked) {
    console.log(`  ${p.slug}`);
    console.log(`    cluster: ${p.pedagogical_cluster}`);
    console.log(`    drift: ${p.taxonomy_drift ?? false}`);
    console.log(`    preview: ${(p.instruction_preview ?? '').slice(0, 120)}`);
  }
}
