#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g08 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g08
 *   npm run catalog:export-lote -- --lote=imunizacao-g08 --slugs-file=data/catalog-migration/imunizacao-g08/g08-slugs.json
 */
...
function loadExcludeSlugs(): Set<string> {
  ...
  // Só lotes handcraft já exportados/aplicados — NÃO usar imunizacao-exclude-done
  // (lista legado/híbrido quase total; g08+ seleciona do cluster report).
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g08';
  ...
  const slugsFile = resolve(loteDir, 'g08-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Handcraft golden-v1 calendário — erro reproduzível no concept_map + mirror danger_zone. Rodar export-lote depois deste plano.',
    workflow: [
      'npm run plan:imunizacao-g08',
      'npm run catalog:export-lote -- --lote=imunizacao-g08 --slugs-file=data/catalog-migration/imunizacao-g08/g08-slugs.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g08 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g08 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g08',
      'npm run capture:questao-review -- --lote=imunizacao-g08',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g08] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g08] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g08] próximo: npm run catalog:export-lote -- --lote=${lote} --slugs-file=data/catalog-migration/${lote}/g08-slugs.json`,
  );
}
