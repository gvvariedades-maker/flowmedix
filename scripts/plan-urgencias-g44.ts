#!/usr/bin/env tsx
/**
 * Seleciona 8 slugs para urgencias-g44 — 15º lote cauda urgencias_generico.
 *
 *   npx tsx scripts/plan-urgencias-g44.ts
 */
...
function main() {
  ...
  if (picked.length < LIMIT) {
    console.error(`[plan:urgencias-g44] pool insuficiente: ${picked.length}/${LIMIT}`);
    process.exit(1);
  }
  ...
  console.log(`[plan:urgencias-g44] pool=${pool.length} picked=${picked.length}`);
