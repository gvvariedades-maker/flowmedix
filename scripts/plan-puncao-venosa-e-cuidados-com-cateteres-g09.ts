#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g09 (últimos 3 slugs cauda puncao_generico).
 *
...
function main(): void {
  ...
  const manifest = {
    ...
    filters: {
      ...
      note: picked.length < requested ? 'lote parcial — cauda generico esgotada' : undefined,
    },
    slugs: picked,
  };
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 cauda — puncao_generico (últimos slugs)',
  ...
  console.log(`[plan:puncao-g09] lote=${LOTE} slugs=${picked.length} pool=${pool.length} remaining=${pool.length - picked.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:puncao-g09] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
