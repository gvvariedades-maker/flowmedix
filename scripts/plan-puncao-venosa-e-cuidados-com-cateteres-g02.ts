#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g02 (P0 puncao_dispositivo).
 *
...
const P0_CLUSTER = 'Dispositivo / calibre / jelco';
...
function main(): void {
  ...
  if (picked.length < batchSize) {
    throw new Error(
      `Só ${picked.length} slugs P0 dispositivo disponíveis (pool=${pool.length}, excluídos=${exclude.size})`,
    );
  }
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 — puncao_dispositivo (Dispositivo / calibre / jelco)',
    slug_count: picked.length,
    pedagogical_branch_target: BRANCH,
    anchors: ['examples/questao-premium-gama-puncao-scalp-jelco-calibre.json'],
    anchor_slug: 'avancasp-enfermagem-puncao-venosa-e-cuidados-com-cateteres-1779340254185-3',
    workflow: [
      'npm run plan:puncao-venosa-e-cuidados-com-cateteres-g02',
  ...
  console.log(`[plan:puncao-g02] lote=${LOTE} slugs=${picked.length} pool=${pool.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:puncao-g02] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
