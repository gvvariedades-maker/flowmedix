#!/usr/bin/env tsx
/**
 * Seleciona slugs para cuidados-na-administracao-de-medicamentos-g13 (cauda pós-g12).
 *
 *   npm run plan:cuidados-na-administracao-de-medicamentos-g13
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'cuidados-na-administracao-de-medicamentos-g13';
    ...
    JSON.stringify(
      {
        ...
        status: 'planned',
        priority: 'P1 — cauda cam_generico (lote 4/5)',
  ...
  console.log(`[plan:cuidados-g13] lote=${lote} slugs=${picked.length} pool_remaining=${pool.length}`);
