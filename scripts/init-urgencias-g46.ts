#!/usr/bin/env tsx
/**
 * Inicializa urgencias-g46 — 8 slugs · 17º lote urgencias_generico (cauda cluster).
 * Slugs selecionados por: npx tsx scripts/plan-urgencias-g46.ts
 */
...
function main() {
  ...
  if (!existsSync(planManifest)) {
    console.error(`[init:urgencias-g46] Rode antes: npx tsx scripts/plan-urgencias-g46.ts`);
    process.exit(1);
  }
  const { slugs } = JSON.parse(readFileSync(planManifest, 'utf8')) as { slugs: string[] };
  if (slugs.length !== 8) {
    console.error(`[init:urgencias-g46] manifest deve ter 8 slugs, tem ${slugs.length}`);
    process.exit(1);
  }
  ...
  for (const slug of slugs) {
    ...
    if (!existsSync(src)) {
      console.error(`[init:urgencias-g46] MISSING ${src}`);
      process.exit(1);
    }
    copyFileSync(src, dest);
    console.log(`[init:urgencias-g46] copied ${slug}`);
  }
    ...
    `${JSON.stringify(
      {
        ...
        filters: {
          ...
          batch: 'g46-generico-tail-batch-8',
        },
        slugs,
      },
      null,
    ...
    `${JSON.stringify(
      {
        ...
        notes: 'g46 — 17º lote cauda generico · pending handcraft · pool tail 14→6 após g46',
      },
      null,
      2,
    )}\n`,
  ...
  console.log(`[init:urgencias-g46] manifest + lote-meta OK (${slugs.length} slugs)`);
}
