#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g45 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g45
 *   npm run catalog:export-lote -- --lote=imunizacao-g45 --from-manifest=data/catalog-migration/imunizacao-g45/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g45';
  ...
  // Tail P0: cluster calendário esgotado (~300/302) — backfill Default imunização
  ...
  const slugsFile = resolve(loteDir, 'g45-slugs.json');
  ...
  const manifest = {
    ...
    filters: {
      ...
      mix: 'infantil+adulto/gestante/HPV alternado; fallback Default imunização se cluster esgotado',
    },
    slugs: picked,
  };
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — tail P0 calendário: 2 infantil Fundatec (cluster) + 6 Default imunização (backfill pós-esgotamento g01–g44). Handcraft golden-v1 calendário. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g45',
      'npm run catalog:export-lote -- --lote=imunizacao-g45 --from-manifest=data/catalog-migration/imunizacao-g45/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g45 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g45 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g45',
      'npm run validate:goldens -- --strict --lote=imunizacao-g45',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g45] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g45] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g45] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
