#!/usr/bin/env tsx
/**
 * Inicializa urgencias-g36 — 8 slugs · 7º lote urgencias_generico (cauda cluster).
 * Slugs selecionados por: npx tsx scripts/plan-urgencias-g36.ts
 */
...
function main() {
  ...
  if (!existsSync(planManifest)) {
    console.error(`[init:urgencias-g36] Rode antes: npx tsx scripts/plan-urgencias-g36.ts`);
    process.exit(1);
  }
  const { slugs } = JSON.parse(readFileSync(planManifest, 'utf8')) as { slugs: string[] };
  if (slugs.length !== 8) {
    console.error(`[init:urgencias-g36] manifest deve ter 8 slugs, tem ${slugs.length}`);
    process.exit(1);
  }
  ...
  for (const slug of slugs) {
    ...
    if (!existsSync(src)) {
      console.error(`[init:urgencias-g36] MISSING ${src}`);
      process.exit(1);
    }
    copyFileSync(src, dest);
    console.log(`[init:urgencias-g36] copied ${slug}`);
  }
    ...
    `${JSON.stringify(
      {
        ...
        filters: {
          ...
          batch: 'g36-generico-tail-batch-7',
        },
        slugs,
      },
      null,
    ...
    `${JSON.stringify(
      {
        ...
        notes: 'g36 — 7º lote cauda urgencias_generico · inferência por enunciado (g30–g35 pattern)',
      },
      null,
      2,
    )}\n`,
  ...
  console.log(`[init:urgencias-g36] manifest + lote-meta OK (${slugs.length} slugs)`);
}
