#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g36 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g36
 *   npm run catalog:export-lote -- --lote=imunizacao-g36 --from-manifest=data/catalog-migration/imunizacao-g36/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g36';
  ...
  const slugsFile = resolve(loteDir, 'g36-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — slugs sem overlap com g01–g35 (incl. g11/g14–g35 handcraft_ready/planned). Handcraft golden-v1 calendário. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g36',
      'npm run catalog:export-lote -- --lote=imunizacao-g36 --from-manifest=data/catalog-migration/imunizacao-g36/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g36 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g36 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g36',
      'npm run validate:goldens -- --strict --lote=imunizacao-g36',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g36] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g36] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g36] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
