#!/usr/bin/env tsx
/**
 * Seleciona 8 slugs para urgencias-g38 — 9º lote cauda urgencias_generico.
 *
 *   npx tsx scripts/plan-urgencias-g38.ts
 */
...
function main() {
  ...
  if (picked.length < LIMIT) {
    console.error(`[plan:urgencias-g38] pool insuficiente: ${picked.length}/${LIMIT}`);
    process.exit(1);
  }
  ...
  console.log(`[plan:urgencias-g38] pool=${pool.length} picked=${picked.length}`);
