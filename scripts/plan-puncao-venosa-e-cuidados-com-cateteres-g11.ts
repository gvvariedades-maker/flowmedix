#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g11 (fim P0 puncao_flebite — 3 slugs).
 *
...
function main(): void {
  ...
  const manifest = {
    ...
    filters: {
      ...
      note: pool.length - picked.length === 0 ? 'cluster flebite esgotado' : undefined,
    },
    slugs: picked,
  };
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 — puncao_flebite (fim do cluster)',
  ...
  console.log(`[plan:puncao-g11] lote=${LOTE} slugs=${picked.length} pool=${pool.length} remaining=${pool.length - picked.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:puncao-g11] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
