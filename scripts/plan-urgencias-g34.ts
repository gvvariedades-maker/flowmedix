#!/usr/bin/env tsx
/**
 * Seleciona 8 slugs para urgencias-g34 — 5º lote cauda urgencias_generico.
 *
 *   npx tsx scripts/plan-urgencias-g34.ts
 */
...
function main() {
  ...
  if (picked.length < LIMIT) {
    console.error(`[plan:urgencias-g34] pool insuficiente: ${picked.length}/${LIMIT}`);
    process.exit(1);
  }
  ...
  console.log(`[plan:urgencias-g34] pool=${pool.length} picked=${picked.length}`);
