#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g03 (P0 puncao_exceto).
 *
...
const P0_CLUSTER = 'EXCETO — técnica / conduta';
...
function main(): void {
  ...
  if (picked.length < batchSize) {
    throw new Error(
      `Só ${picked.length} slugs P0 exceto disponíveis (pool=${pool.length}, excluídos=${exclude.size})`,
    );
  }
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 — puncao_exceto (EXCETO técnica / conduta)',
    slug_count: picked.length,
    pedagogical_branch_target: BRANCH,
    anchors: ['examples/questao-premium-cev-urca-puncao-exceto-med-endovenosa.json'],
    anchor_slug: 'cev-urca-enfermagem-puncao-venosa-e-cuidados-com-cateteres-1779340232037-0',
    workflow: [
      'npm run plan:puncao-venosa-e-cuidados-com-cateteres-g03',
  ...
  console.log(`[plan:puncao-g03] lote=${LOTE} slugs=${picked.length} pool=${pool.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:puncao-g03] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
