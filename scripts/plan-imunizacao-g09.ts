#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g09 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g09
 *   npm run catalog:export-lote -- --lote=imunizacao-g09 --slugs-file=data/catalog-migration/imunizacao-g09/g09-slugs.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g09';
  ...
  const slugsFile = resolve(loteDir, 'g09-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Handcraft golden-v1 calendário g09 — erro reproduzível no concept_map + mirror danger_zone. Exclui slugs g01–g08.',
    workflow: [
      'npm run plan:imunizacao-g09',
      'npm run catalog:export-lote -- --lote=imunizacao-g09 --from-manifest=data/catalog-migration/imunizacao-g09/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g09 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g09 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g09',
      'npm run capture:questao-review -- --lote=imunizacao-g09',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g09] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g09] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g09] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
