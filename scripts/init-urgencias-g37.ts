#!/usr/bin/env tsx
/**
 * Inicializa urgencias-g37 — 8 slugs · 8º lote urgencias_generico (cauda cluster).
 * Slugs selecionados por: npx tsx scripts/plan-urgencias-g37.ts
 */
...
function main() {
  ...
  if (!existsSync(planManifest)) {
    console.error(`[init:urgencias-g37] Rode antes: npx tsx scripts/plan-urgencias-g37.ts`);
    process.exit(1);
  }
  const { slugs } = JSON.parse(readFileSync(planManifest, 'utf8')) as { slugs: string[] };
  if (slugs.length !== 8) {
    console.error(`[init:urgencias-g37] manifest deve ter 8 slugs, tem ${slugs.length}`);
    process.exit(1);
  }
  ...
  for (const slug of slugs) {
    ...
    if (!existsSync(src)) {
      console.error(`[init:urgencias-g37] MISSING ${src}`);
      process.exit(1);
    }
    copyFileSync(src, dest);
    console.log(`[init:urgencias-g37] copied ${slug}`);
  }
    ...
    `${JSON.stringify(
      {
        ...
        filters: {
          ...
          batch: 'g37-generico-tail-batch-8',
        },
        slugs,
      },
      null,
    ...
    `${JSON.stringify(
      {
        ...
        notes: 'g37 — 8º lote cauda urgencias_generico · inferência por enunciado (g30–g36 pattern)',
      },
      null,
      2,
    )}\n`,
  ...
  console.log(`[init:urgencias-g37] manifest + lote-meta OK (${slugs.length} slugs)`);
}
