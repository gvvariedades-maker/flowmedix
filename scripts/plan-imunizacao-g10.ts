#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g10 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g10
 *   npm run catalog:export-lote -- --lote=imunizacao-g10 --slugs-file=data/catalog-migration/imunizacao-g10/g10-slugs.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g10';
  ...
  const slugsFile = resolve(loteDir, 'g10-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Handcraft golden-v1 calendário g10 — infantil + adolescente/adulto/gestante/HPV. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g10',
      'npm run catalog:export-lote -- --lote=imunizacao-g10 --slugs-file=data/catalog-migration/imunizacao-g10/g10-slugs.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g10 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g10 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g10',
      'npm run capture:questao-review -- --lote=imunizacao-g10',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g10] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g10] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g10] próximo: npm run catalog:export-lote -- --lote=${lote} --slugs-file=data/catalog-migration/${lote}/g10-slugs.json`,
  );
}
