#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g32 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g32
 *   npm run catalog:export-lote -- --lote=imunizacao-g32 --from-manifest=data/catalog-migration/imunizacao-g32/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g32';
  ...
  const slugsFile = resolve(loteDir, 'g32-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — slugs sem overlap com g01–g31 (incl. g11/g14–g31 handcraft_ready/planned). Handcraft golden-v1 calendário. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g32',
      'npm run catalog:export-lote -- --lote=imunizacao-g32 --from-manifest=data/catalog-migration/imunizacao-g32/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g32 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g32 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g32',
      'npm run validate:goldens -- --strict --lote=imunizacao-g32',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g32] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g32] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g32] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
