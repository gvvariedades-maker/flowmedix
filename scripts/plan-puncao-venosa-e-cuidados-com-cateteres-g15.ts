#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g15 (cauda final — 10 slugs restantes).
 *
...
const CLUSTER_PRIORITY = [
  { cluster: 'Cálculo / dose / tempo numérico', branch: 'puncao_generico' },
  { cluster: 'V/F — assertivas I/II/III', branch: 'puncao_generico' },
  { cluster: 'Acesso venoso central', branch: 'puncao_ipcs_cvc' },
  { cluster: 'Punção venosa periférica', branch: 'puncao_periferica_antissepsia' },
  { cluster: 'Acesso arterial / PAM', branch: 'puncao_generico' },
  { cluster: 'Medicação endovenosa — técnica', branch: 'puncao_generico' },
] as const;
...
function main(): void {
  const requested = Number(parseArg('size') ?? '10');
  ...
  if (picked.length === 0) {
    throw new Error('Nenhum slug disponível para g15');
  }
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P1 — cauda final (calc + VF + CVC + periférica + arterial + EV)',
  ...
  console.log(`[plan:puncao-g15] lote=${LOTE} slugs=${picked.length}`);
  for (const p of picked) console.log(`  · [${p.branch}] ${p.slug}`);
  console.log(
    `[plan:puncao-g15] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
