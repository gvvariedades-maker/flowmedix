#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g16 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g16
 *   npm run catalog:export-lote -- --lote=imunizacao-g16 --from-manifest=data/catalog-migration/imunizacao-g16/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g16';
  ...
  const slugsFile = resolve(loteDir, 'g16-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — slugs sem overlap com g01–g15 (incl. g11/g14/g15 handcraft_ready). Handcraft golden-v1 calendário. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g16',
      'npm run catalog:export-lote -- --lote=imunizacao-g16 --from-manifest=data/catalog-migration/imunizacao-g16/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g16 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g16 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g16',
      'npm run capture:questao-review -- --lote=imunizacao-g16',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g16] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g16] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g16] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
