#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g42 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g42
 *   npm run catalog:export-lote -- --lote=imunizacao-g42 --from-manifest=data/catalog-migration/imunizacao-g42/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g42';
  ...
  const exclude = loadExcludeSlugs(
    lote === 'imunizacao-g42' ? [lote, 'imunizacao-g43'] : lote,
  );
  ...
  const slugsFile = resolve(loteDir, 'g42-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — último lote P0 calendário (2 infantil Fundatec + 6 adulto/HPV). g43 planejado fora de ordem — slugs reatribuídos ao g42. Handcraft golden-v1 calendário.',
    workflow: [
      'npm run plan:imunizacao-g42',
      'npm run catalog:export-lote -- --lote=imunizacao-g42 --from-manifest=data/catalog-migration/imunizacao-g42/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g42 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g42 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g42',
      'npm run validate:goldens -- --strict --lote=imunizacao-g42',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g42] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g42] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g42] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
