#!/usr/bin/env tsx
/**
 * Seleciona slugs para cuidados-na-administracao-de-medicamentos-g03 (P0 cam_certos_vf_caso — cauda 10).
 *
 *   npm run plan:cuidados-na-administracao-de-medicamentos-g03
 *   npm run catalog:export-lote -- --lote=cuidados-na-administracao-de-medicamentos-g03 --from-manifest=data/catalog-migration/cuidados-na-administracao-de-medicamentos-g03/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'cuidados-na-administracao-de-medicamentos-g03';
  ...
  if (picked.length === 0) {
    throw new Error(`Nenhum slug disponível (excluídos ${exclude.size}). pool=${pool.length}`);
  }
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 — cam_certos_vf_caso (cauda cluster 10 slugs)',
    ...
    workflow: [
      'npm run plan:cuidados-na-administracao-de-medicamentos-g03',
      'npm run catalog:export-lote -- --lote=cuidados-na-administracao-de-medicamentos-g03 --from-manifest=data/catalog-migration/cuidados-na-administracao-de-medicamentos-g03/manifest.json',
      'Handcraft golden-v1 — cam-certos-* 4/4',
      'npm run validate:goldens -- --lote=cuidados-na-administracao-de-medicamentos-g03 --strict',
      'npm run audit:questao-readiness -- --lote=cuidados-na-administracao-de-medicamentos-g03 --strict-v3-pedagogy',
      'npm run audit:slug-alignment -- --lote=cuidados-na-administracao-de-medicamentos-g03 --strict',
      'npm run audit:anchor-review -- --lote=cuidados-na-administracao-de-medicamentos-g03 --record-pass --reviewer=agent --skip-capture',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:cuidados-g03] lote=${lote} slugs=${picked.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:cuidados-g03] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
