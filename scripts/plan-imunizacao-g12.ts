#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g12 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g12
 *   npm run catalog:export-lote -- --lote=imunizacao-g12 --from-manifest=data/catalog-migration/imunizacao-g12/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g12';
  ...
  const slugsFile = resolve(loteDir, 'g12-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Handcraft golden-v1 calendário g12 — infantil + adolescente/adulto/gestante/HPV. Erro reproduzível no concept_map + mirror danger_zone.',
    workflow: [
      'npm run plan:imunizacao-g12',
      'npm run catalog:export-lote -- --lote=imunizacao-g12 --from-manifest=data/catalog-migration/imunizacao-g12/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g12 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g12 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g12',
      'npm run capture:questao-review -- --lote=imunizacao-g12',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g12] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g12] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g12] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
