 ...
 * Uso: npx tsx scripts/plan-epidemiologia-e-vigilancia-epidemiologica-batches.ts
 */
import { mkdirSync, readFileSync, writeFileSync, existsSync, cpSync } from 'node:fs';
import { resolve } from 'node:path';

const LOTE_SIZE = 8;
const COMPLETO = 'epidemiologia-e-vigilancia-epidemiologica-completo';
const PREFIX = 'epidemiologia-e-vigilancia-epidemiologica';
...
function main() {
  ...
  writeFileSync(
    ...
    `${JSON.stringify({ total: slugs.length, lote_size: LOTE_SIZE, batches: plan.length, plan }, null, 2)}\n`,
  );
  console.log(`[plan:epidemiologia] total=${slugs.length} batches=${plan.length} size=${LOTE_SIZE}`);
  console.log(`[plan:epidemiologia] first=${plan[0]?.lote} last=${plan[plan.length - 1]?.lote}`);
}
