#!/usr/bin/env tsx
/**
 * Planeja puncao-venosa-e-cuidados-com-cateteres-g12 (P0 Antissepsia na punção — 8 slugs).
 *
...
const P0_CLUSTER = 'Antissepsia na punção';
...
function main(): void {
  ...
  if (picked.length === 0) {
    throw new Error(`Nenhum slug antissepsia disponível (pool=${pool.length}, excluídos=${exclude.size})`);
  }
  ...
  const manifest = {
    ...
    filters: {
      ...
      note: pool.length - picked.length === 0 ? 'cluster antissepsia esgotado' : undefined,
    },
    slugs: picked,
  };
  ...
  const loteMeta = {
    ...
    status: 'planned',
    priority: 'P0 — puncao_periferica_antissepsia (Antissepsia na punção)',
    slug_count: picked.length,
    pedagogical_branch_target: BRANCH,
    anchors: ['examples/questao-premium-funpar-puncao-tecnica-periferica.json'],
    anchor_slug: 'gama-enfermagem-puncao-venosa-e-cuidados-com-cateteres-1779340232037-3',
    workflow: [
      'npm run plan:puncao-venosa-e-cuidados-com-cateteres-g12',
  ...
  console.log(`[plan:puncao-g12] lote=${LOTE} slugs=${picked.length} pool=${pool.length} remaining=${pool.length - picked.length}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:puncao-g12] próximo: npm run catalog:export-lote -- --lote=${LOTE} --from-manifest=data/catalog-migration/${LOTE}/manifest.json`,
  );
}
