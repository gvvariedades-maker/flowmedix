 ...
 * Uso: npx tsx scripts/plan-atencao-basica-saude-da-familia-batches.ts
 */
import { mkdirSync, readFileSync, writeFileSync, existsSync, cpSync } from 'node:fs';
import { resolve } from 'node:path';

const LOTE_SIZE = 8;
const COMPLETO = 'atencao-basica-saude-da-familia-completo';
const PREFIX = 'atencao-basica-saude-da-familia';
...
function main() {
  ...
  console.log(`[plan:atencao-basica] total=${slugs.length} batches=${plan.length} size=${LOTE_SIZE}`);
  console.log(`[plan:atencao-basica] first=${plan[0]?.lote} last=${plan[plan.length - 1]?.lote}`);
}
