#!/usr/bin/env python3
"""Extrai todas as questões de Denotação, conotação e figuras de linguagem do caderno PT interno."""
...
OUT = ROOT / "data/catalog-migration/denotacao-conotacao-completo/extracted-source.json"
...
CARD_TITLE = "Denotação, conotação e figuras de linguagem"
DENOTACAO_ASSUNTOS = {
...
def suggest_slug(banca: str, instruction: str, tec_id: str) -> str:
    ...
    return f"{banca_slug}-denotacao-{hint}-{tec_id}"[:90].rstrip("-")
...
def load_target_tec() -> list[str]:
    ...
    for q in data["questions_index"]:
        assunto = fix_encoding(q.get("source_assunto_tec", ""))
        if assunto in DENOTACAO_ASSUNTOS:
            out.append(q["tec_id"])
    return out
