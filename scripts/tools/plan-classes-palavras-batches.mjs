...
const src = JSON.parse(
  readFileSync('data/catalog-migration/classes-de-palavras-completo/extracted-source.json', 'utf8'),
);
const slugs = src.map((q) => q.slug);
const prefix = 'classes-de-palavras';
const sub = 'Classes de palavras';
const top = 'Língua Portuguesa';
const branch = 'pt_classes_palavras';
const batchSize = 8;

writeFileSync(
  'data/catalog-migration/classes-de-palavras-completo/manifest.json',
  JSON.stringify(
    {
    ...
    JSON.stringify(
      {
        ...
        status: 'planned',
        slugs: chunk,
        note: 'Fase 0 pipeline Classes de palavras 2026-07-20',
      },
      null,
      2,
    ) + '\n',
