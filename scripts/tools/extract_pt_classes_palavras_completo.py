#!/usr/bin/env python3
"""Extrai todas as questões de Classes de palavras do caderno PT interno."""
...
OUT = ROOT / "data/catalog-migration/classes-de-palavras-completo/extracted-source.json"
...
def suggest_slug(banca: str, instruction: str, tec_id: str) -> str:
    ...
    return f"{banca_slug}-classes-{hint}-{tec_id}"[:90].rstrip("-")
...
def load_target_tec() -> list[str]:
    ...
    return [
        ...
        if any(a in q.get("source_assunto_tec", "") for a in CLASSES_ASSUNTOS)
    ]
