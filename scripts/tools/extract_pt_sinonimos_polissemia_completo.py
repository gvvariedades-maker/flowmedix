#!/usr/bin/env python3
"""Extrai todas as questões de Sinônimos, antônimos e polissemia do caderno PT interno."""
...
OUT = ROOT / "data/catalog-migration/sinonimos-antonimos-e-polissemia-completo/extracted-source.json"
...
CARD_TITLE = "Sinônimos, antônimos e polissemia"
SINONIMOS_ASSUNTOS = {
...
def suggest_slug(banca: str, instruction: str, tec_id: str) -> str:
    ...
    return f"{banca_slug}-sinonimos-{hint}-{tec_id}"[:90].rstrip("-")
...
def load_target_tec() -> list[str]:
    ...
    for q in data["questions_index"]:
        ...
        if assunto in SINONIMOS_ASSUNTOS:
            out.append(q["tec_id"])
    return out
