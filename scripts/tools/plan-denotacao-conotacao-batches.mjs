...
const src = JSON.parse(
  readFileSync('data/catalog-migration/denotacao-conotacao-completo/extracted-source.json', 'utf8'),
);
const slugs = src.map((q) => q.slug);
const prefix = 'denotacao-conotacao';
const sub = 'Denotação, conotação e figuras de linguagem';
const top = 'Língua Portuguesa';
const branch = 'pt_denotacao_conotacao';
const batchSize = 8;

writeFileSync(
  'data/catalog-migration/denotacao-conotacao-completo/manifest.json',
  JSON.stringify(
    {
      ...
      source: 'cluster:lingua-portuguesa + extract_pt_denotacao_conotacao_completo.py',
    },
    null,
    2,
  ) + '\n',
    ...
    JSON.stringify(
      {
        ...
        status: 'planned',
        slugs: chunk,
        note: 'Fase 0 bootstrap — Denotação, conotação e figuras de linguagem 2026-07-23',
      },
      null,
      2,
    ) + '\n',
