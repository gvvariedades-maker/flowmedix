#!/usr/bin/env python3
"""Extrai todas as questões de Concordância verbal e nominal do caderno PT interno."""
...
OUT = ROOT / "data/catalog-migration/concordancia-verbal-e-nominal-completo/extracted-source.json"
...
def parse_banca_orgao(banca_line: str) -> tuple[str, str, str]:
    """Heurística: BANCA – Cargo (Órgão) ANO"""
    banca = "CADERNO"
    orgao = ""
    ano = "2026"
...
def suggest_slug(banca: str, instruction: str, tec_id: str) -> str:
    ...
    return f"{banca_slug}-concordancia-{hint}-{tec_id}"[:90].rstrip("-")
...
def load_target_tec() -> list[str]:
    ...
    return [
        ...
        if "Concord" in q.get("source_assunto_tec", "")
    ]
            ...
            out.append(
                {
                    ...
                    if header_match
                    else "Concordância (Verbal e Nominal)",
