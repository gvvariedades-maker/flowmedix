#!/usr/bin/env tsx
/**
 * Seleciona slugs para imunizacao-g52 (calendário P0) a partir do cluster report.
 *
 *   npm run plan:imunizacao-g52
 *   npm run catalog:export-lote -- --lote=imunizacao-g52 --from-manifest=data/catalog-migration/imunizacao-g52/manifest.json
 */
...
function main(): void {
  const lote = parseArg('lote') ?? 'imunizacao-g52';
  ...
  const slugsFile = resolve(loteDir, 'g52-slugs.json');
  ...
  const loteMeta = {
    ...
    notes:
      'Planejado 2026-07-03 — tail P0 calendário pós g01–g51: slugs via cluster + manifest completo (incl. processo-de-enfermagem/geral-imunizacao). Handcraft golden-v1 calendário.',
    workflow: [
      'npm run plan:imunizacao-g52',
      'npm run catalog:export-lote -- --lote=imunizacao-g52 --from-manifest=data/catalog-migration/imunizacao-g52/manifest.json',
      'Handcraft por slug — gramática imunizacao-pedagogy-errors.json',
      'npm run audit:questao-readiness -- --lote=imunizacao-g52 --strict-v2-pedagogy',
      'npm run audit:slug-alignment -- --lote=imunizacao-g52 --strict',
      'npm run audit:numeric-factcheck -- --lote=imunizacao-g52',
      'npm run validate:goldens -- --strict --lote=imunizacao-g52',
    ],
  };
  writeFileSync(resolve(loteDir, 'lote-meta.json'), JSON.stringify(loteMeta, null, 2), 'utf8');

  console.log(`[plan:imunizacao-g52] lote=${lote} slugs=${picked.length}`);
  console.log(`[plan:imunizacao-g52] slugs_file=${slugsFile}`);
  for (const s of picked) console.log(`  · ${s}`);
  console.log(
    `[plan:imunizacao-g52] próximo: npm run catalog:export-lote -- --lote=${lote} --from-manifest=data/catalog-migration/${lote}/manifest.json`,
  );
}
