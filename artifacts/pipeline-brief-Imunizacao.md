...
```bash
npm run audit:subtopico-inventory -- --subtopico="Imunização"
# Se drift: conversa Classify: Imunização
# Subtópico novo: conversa Mapeamento L3: Imunização
npm run catalog:export-lote -- --lote=<pacote>-completo --subtopico="Imunização" --limit=10000
...
|-------|-------------------|
| Taxonomia | `Classify: Imunização` (se inventário com drift) |
| L3 | `Mapeamento L3: Imunização` (subtópico novo ou sem cluster) |
| Brief handcraft | `npm run handcraft:brief -- --subtopico="Imunização"` |
