# Pipeline completo — Sinônimos, antônimos e polissemia
...
```bash
npm run audit:subtopico-inventory -- --subtopico="Sinônimos, antônimos e polissemia"
# Se drift: conversa Classify: Sinônimos, antônimos e polissemia
# Subtópico novo: conversa Mapeamento L3: Sinônimos, antônimos e polissemia
npm run catalog:export-lote -- --lote=<pacote>-completo --subtopico="Sinônimos, antônimos e polissemia" --limit=10000
# Criar entrada em handcraft-registry.json se ausente
# npm run cluster:<pacote> (se existir)
npm run audit:golden-anchor-gate -- --subtopico="Sinônimos, antônimos e polissemia"
# Se gate=block: Criar âncoras: Sinônimos, antônimos e polissemia (skill avant-golden-anchor-bootstrap)
npm run anchor:brief -- --subtopico="Sinônimos, antônimos e polissemia"
```
...
|-------|-------------------|
| Taxonomia | `Classify: Sinônimos, antônimos e polissemia` (se inventário com drift) |
| L3 | `Mapeamento L3: Sinônimos, antônimos e polissemia` (subtópico novo ou sem cluster) |
| Brief handcraft | `npm run handcraft:brief -- --subtopico="Sinônimos, antônimos e polissemia"` |
