# Handcraft briefing — Enfermagem em Central de Material e Esterilização (CME)
...
|-------|-------|
| pacote_prefix | `cme` |
| status | applied (35/35 slugs) |
| manifest | `data/catalog-migration/cme-completo/manifest.json` |
| lote_pattern | `cme-g{NN}` |
| lote_size | 8 |
| anchor_glob | `examples/questao-premium-*-cme-*.json` |
| guideline | `lib/guidelines/cme.ts` |
| handcraft_meta | `data/catalog-migration/cme-g05/lote-meta.json` |
...
```bash
npm run catalog:export-lote -- --lote=cme-completo --subtopico="Enfermagem em Central de Material e Esterilização (CME)" --limit=10000
# Handcraft → data/catalog-migration/cme-g01/questions/*.json
npm run validate:goldens -- --lote=cme-g01 --strict
npm run audit:questao-readiness -- --lote=cme-g01
