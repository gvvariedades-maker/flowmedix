# Pipeline completo — Orações coordenadas e subordinadas
...
```bash
npm run audit:subtopico-inventory -- --subtopico="Orações coordenadas e subordinadas"
# Se drift: conversa Classify: Orações coordenadas e subordinadas
# Subtópico novo: conversa Mapeamento L3: Orações coordenadas e subordinadas
npm run catalog:export-lote -- --lote=<pacote>-completo --subtopico="Orações coordenadas e subordinadas" --limit=10000
# Criar entrada em handcraft-registry.json se ausente
# npm run cluster:<pacote> (se existir)
npm run audit:golden-anchor-gate -- --subtopico="Orações coordenadas e subordinadas"
# Se gate=block: Criar âncoras: Orações coordenadas e subordinadas (skill avant-golden-anchor-bootstrap)
npm run anchor:brief -- --subtopico="Orações coordenadas e subordinadas"
```
...
|-------|-------------------|
| Taxonomia | `Classify: Orações coordenadas e subordinadas` (se inventário com drift) |
| L3 | `Mapeamento L3: Orações coordenadas e subordinadas` (subtópico novo ou sem cluster) |
| Brief handcraft | `npm run handcraft:brief -- --subtopico="Orações coordenadas e subordinadas"` |
