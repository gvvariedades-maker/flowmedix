# Pipeline completo — Denotação, conotação e figuras de linguagem
...
```bash
npm run audit:subtopico-inventory -- --subtopico="Denotação, conotação e figuras de linguagem"
# Se drift: conversa Classify: Denotação, conotação e figuras de linguagem
# Subtópico novo: conversa Mapeamento L3: Denotação, conotação e figuras de linguagem
npm run catalog:export-lote -- --lote=<pacote>-completo --subtopico="Denotação, conotação e figuras de linguagem" --limit=10000
# Criar entrada em handcraft-registry.json se ausente
# npm run cluster:<pacote> (se existir)
npm run audit:golden-anchor-gate -- --subtopico="Denotação, conotação e figuras de linguagem"
# Se gate=block: Criar âncoras: Denotação, conotação e figuras de linguagem (skill avant-golden-anchor-bootstrap)
npm run anchor:brief -- --subtopico="Denotação, conotação e figuras de linguagem"
```
...
|-------|-------------------|
| Taxonomia | `Classify: Denotação, conotação e figuras de linguagem` (se inventário com drift) |
| L3 | `Mapeamento L3: Denotação, conotação e figuras de linguagem` (subtópico novo ou sem cluster) |
| Brief handcraft | `npm run handcraft:brief -- --subtopico="Denotação, conotação e figuras de linguagem"` |
