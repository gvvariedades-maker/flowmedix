...
describe('isBottomNavItemActive', () => {
  ...
  describe('Simulados', () => {
    ...
    it('ativa em /desempenho/simulados (drawer sidebar)', () => {
      expect(isBottomNavItemActive('/desempenho/simulados', BOTTOM_NAV_HREFS.simulados)).toBe(true);
      expect(
        isBottomNavItemActive('/desempenho/simulados/detalhe', BOTTOM_NAV_HREFS.simulados),
      ).toBe(true);
    });

    it('não ativa outras rotas de desempenho', () => {
      expect(isBottomNavItemActive('/desempenho', BOTTOM_NAV_HREFS.simulados)).toBe(false);
    });
  });

  describe('Progresso', () => {
    it('ativa em /progresso e /analytics (exact)', () => {
      expect(isBottomNavItemActive('/progresso', BOTTOM_NAV_HREFS.progresso)).toBe(true);
      expect(isBottomNavItemActive('/analytics', BOTTOM_NAV_HREFS.progresso)).toBe(true);
    });

    it('não ativa subrotas de analytics', () => {
      expect(isBottomNavItemActive('/analytics/detalhe', BOTTOM_NAV_HREFS.progresso)).toBe(false);
    });
  });
...
describe('isBottomNavMaisActive', () => {
  it('ativa rotas exclusivas do drawer', () => {
    ...
    expect(isBottomNavMaisActive('/revisoes-hoje')).toBe(true);
