-- RPC: página da vitrine com entitlements, filtros, histórico agregado e paginação por assunto.
-- Espelha lib/vitrine/service.ts + lib/concursos/entitlements.ts + lib/vitrine/buildGroups.ts.
  ...
  filtered_modulos AS (
    ...
    WHERE (
      ...
      AND (
        p_q IS NULL
        OR btrim(p_q) = ''
        OR lower(coalesce(am.titulo_aula, '')) LIKE '%' || lower(btrim(p_q)) || '%'
        OR lower(coalesce(am.modulo_nome, '')) LIKE '%' || lower(btrim(p_q)) || '%'
        OR lower(coalesce(am.banca, '')) LIKE '%' || lower(btrim(p_q)) || '%'
        OR lower(coalesce(am.modulo_slug, '')) LIKE '%' || lower(btrim(p_q)) || '%'
        OR (
          am.avant_codigo IS NOT NULL
          AND (
            am.avant_codigo::text = regexp_replace(lower(btrim(p_q)), '^q-?', '', 'i')
            OR ('q-' || am.avant_codigo::text) LIKE '%' || lower(btrim(p_q)) || '%'
          )
        )
      )
  ),
...
COMMENT ON FUNCTION public.get_vitrine_page(uuid, int, text, text, text) IS
  'Vitrine paginada por titulo_aula: entitlements, filtros banca/assunto/q, histórico agregado e questões nested. Uso: service_role.';
