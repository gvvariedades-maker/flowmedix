-- Multi-selecao banca/assunto: arrays p_bancas / p_assuntos nas RPCs de vitrine e simulado.

DROP FUNCTION IF EXISTS public.get_simulado_question_pool(uuid, integer, text, text, text);
DROP FUNCTION IF EXISTS public.get_simulado_question_pool_count(uuid, text, text, text);
DROP FUNCTION IF EXISTS public.get_vitrine_page(uuid, int, text, text, text);
DROP FUNCTION IF EXISTS public.get_vitrine_facets(uuid, text);
...
BEGIN
  ...
  modulos_enriched AS (
    SELECT
      ...
      fm.modulo_slug,
      fm.modulo_nome,
      coalesce(fm.titulo_aula, 'Sem subtÃ³pico') AS group_key,
      fm.titulo_aula,
      fm.banca,
      fm.created_at,
      fm.avant_codigo,
...
REVOKE EXECUTE ON FUNCTION public.get_simulado_question_pool(uuid, integer, text, text, text, text[], text[]) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_simulado_question_pool(uuid, integer, text, text, text, text[], text[]) TO service_role;

REVOKE EXECUTE ON FUNCTION public.get_simulado_question_pool_count(uuid, text, text, text, text[], text[]) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_simulado_question_pool_count(uuid, text, text, text, text[], text[]) TO service_role;

REVOKE EXECUTE ON FUNCTION public.get_vitrine_page(uuid, int, text, text, text, text[], text[]) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_vitrine_page(uuid, int, text, text, text, text[], text[]) TO service_role;

REVOKE EXECUTE ON FUNCTION public.get_vitrine_facets(uuid, text, text[]) FROM PUBLIC, anon, authenticated;
GRANT EXECUTE ON FUNCTION public.get_vitrine_facets(uuid, text, text[]) TO service_role;
