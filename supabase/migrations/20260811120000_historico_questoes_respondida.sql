-- P2 Hub Desempenho: distingue tentativa real vs placeholder de "marcar como estudado".
-- concluir-estudo-reverso sem resposta grava respondida=false; registrar-tentativa grava true.
-- Agrega��es de % (vitrine RPC + JS) contam s� respondida=true; estudo_reverso_concluido permanece em todas as linhas.

ALTER TABLE public.historico_questoes
  ADD COLUMN IF NOT EXISTS respondida boolean NOT NULL DEFAULT true;

COMMENT ON COLUMN public.historico_questoes.respondida IS
  'true = aluno escolheu alternativa (conta no % de acerto); false = placeholder de estudo reverso sem resposta.';

CREATE INDEX IF NOT EXISTS idx_historico_questoes_user_respondida
  ON public.historico_questoes (user_id)
  WHERE respondida = true;

-- Placeholders legados do insert em concluir-estudo-reverso (meta DESCONHECIDA/Geral + acertou false).
UPDATE public.historico_questoes
SET respondida = false
WHERE respondida = true
  AND estudo_reverso_concluido IS TRUE
  AND acertou IS FALSE
  AND coalesce(banca, '') = 'DESCONHECIDA'
  AND coalesce(topico, '') = 'Geral'
  AND coalesce(subtopico, '') = 'Geral';

-- get_vitrine_page: % e total_resolvidas s� com respondida=true; BOOL_OR de estudo_reverso inalterado.
DROP FUNCTION IF EXISTS public.get_vitrine_page(uuid, int, text, text, text, text[], text[], text);
